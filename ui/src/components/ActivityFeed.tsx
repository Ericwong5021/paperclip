...
const FILTER_OPTIONS: Array<{ value: FilterValue; labelKey: string }> = [
  ...
  { value: "for-review", labelKey: "inReview" },
  { value: "completed", labelKey: "done" },
];
...
function CollapsedFeedGroup({
  ...
  const actorName = actor?.name
    ?? (group.latestEvent.actorType === "system" ? t("activityFeed.system")
      : group.latestEvent.actorType === "user" ? t("activityFeed.board")
      : t("activityFeed.unknown"));
    ...
    <div>
      ...
      >
        ...
        <span className="flex-1 min-w-0 truncate">
          ...
          <span data-fc="verb" className="ml-1 text-(--hex-959596)">{t("activityFeed.madeUpdates", { count: group.events.length })}</span>
...
export function ActivityFeed({ className }: ActivityFeedProps) {
  ...
  const emptyMessage = useMemo(() => {
    ...
    if (allPaused) {
      return {
        text: t("activityFeed.allPaused"),
        showPulse: false,
      };
    }
    return {
      text: t("activityFeed.runningSoon"),
      showPulse: true,
    };
  }, [agents, t]);
    ...
    <aside className={cn("flex min-h-0 min-w-0 flex-1 flex-col bg-background", className)}>
      ...
      <div className="relative flex shrink-0 items-start justify-between gap-2 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-sm font-semibold">{t("activityFeed.title")}</h3>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={t("activityFeed.groupByTask")}
            ...
            <TooltipContent side="bottom">
              {groupMode === "flat" ? t("activityFeed.groupByTask") : t("activityFeed.showFlat")}
            </TooltipContent>
          </Tooltip>
              ...
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon-sm"
                    className="shrink-0 text-muted-foreground"
                    aria-label={t("activityFeed.filterBy")}
                  >
                    <ListFilter className="h-3.5 w-3.5" />
                  </Button>
                </DropdownMenuTrigger>
              </TooltipTrigger>
              <TooltipContent side="bottom">{t("activityFeed.filterBy")}</TooltipContent>
            </Tooltip>
              ...
              >
                {FILTER_OPTIONS.map(({ value, labelKey }) => (
                  <DropdownMenuRadioItem key={value} value={value}>
                    {t(`activityFeed.filters.${labelKey}`)}
                  </DropdownMenuRadioItem>
                ))}
              </DropdownMenuRadioGroup>
              <DropdownMenuSeparator />
          ...
          <div className="flex min-h-0 flex-1 items-center justify-center p-6 h-full">
            <div className="flex flex-col items-center gap-2 max-w-(--sz-16rem)">
              ...
              <p className="text-center text-sm text-muted-foreground">
                {emptyMessage?.text ?? t("activityFeed.empty")}
              </p>
            </div>
          </div>
        ) : groupMode === "flat" ? (
