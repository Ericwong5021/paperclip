      ...
      <PopoverTrigger asChild>
        ...
        >
          <span className={cn("min-w-0 truncate", !selectedAgent && "text-muted-foreground")}>
            {selectedAgent?.name ?? (placeholder === "Select agent…" ? t("agentManagement.multiSelect.selectAgent") : placeholder)}
          </span>
      ...
      <PopoverContent className="w-(--radix-popover-trigger-width) p-0" align="start">
        <div className="border-b border-border p-3">
          <Input
            ...
            placeholder={t("agentManagement.multiSelect.filterAgents")}
            className="h-8"
            autoFocus
          />
        </div>
        {agents.length === 0 ? (
          <div className="px-3 py-4 text-sm text-muted-foreground">{emptyMessage === "No agents yet." ? t("agentManagement.common.noAgentsYet") : emptyMessage}</div>
        ) : (
          <div className="max-h-60 overflow-y-auto py-1">
            {filteredAgents.map((agent) => (
              <button
                ...
                aria-label={t("agentManagement.multiSelect.selectAgentAria", { name: agent.name })}
                onClick={() => {
                  onChange(agent.id);
                  setOpen(false);
                }}
            ...
            {filteredAgents.length === 0 ? (
              <div className="px-3 py-4 text-sm text-muted-foreground">{t("agentManagement.common.noMatches")}</div>
            ) : null}
          </div>
        )}
      </PopoverContent>
            ...
            <span className="flex min-w-0 items-center">
              ...
              <span className="truncate">
                {triggerLabel ?? (selectedCount === 0
                  ? t("agentManagement.multiSelect.selectAgents")
                  : t(selectedCount === 1 ? "agentManagement.multiSelect.selectedOne" : "agentManagement.multiSelect.selectedMany", { selectedCount }))}
              </span>
            </span>
            <ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground" />
          </Button>
        ...
        {loading ? (
              ...
              return (
                ...
                >
                  <Checkbox
                    ...
                    aria-label={t("agentManagement.multiSelect.allowAgent", { name: agent.name })}
          ...
          <div className="flex items-center justify-between border-t border-border px-3 py-2">
            <span className="text-xs text-muted-foreground">
              {workingAgentIds.size === 0 ? t("agentManagement.multiSelect.noAgentsSelected") : t("agentManagement.multiSelect.selectedCount", { selectedCount: workingAgentIds.size })}
            </span>
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                disabled={pending}
              >
                {staged ? (pending ? t("agentManagement.multiSelect.saving") : t("agentManagement.common.save")) : t("agentManagement.common.done")}
              </Button>
            </div>
          </div>
        </PopoverContent>
      ...
      {showSelectionPreview && selectedAgents.length > 0 ? (
        <div className="space-y-0.5">
          ...
          {selectedAgents.length > 3 ? (
            <p className="px-1.5 pt-0.5 text-xs text-muted-foreground">{t("agentManagement.multiSelect.andMore", { moreCount: selectedAgents.length - 3 })}</p>
          ) : null}
        </div>
      ) : null}
    </div>
