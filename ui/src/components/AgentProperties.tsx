    ...
    <div className="space-y-4">
      <div className="space-y-1">
        ...
        {lastErrorIsActive && agent.errorReason && (
          <PropertyRow label={t("agentDetail.error")}>
            <span className="text-xs text-red-600 dark:text-red-400 break-words min-w-0">
              {agent.errorReason}
            </span>
          </PropertyRow>
        )}
        <PropertyRow label={t("agentDetail.role", { defaultValue: "Role" })}>
      ...
      <div className="space-y-1">
        {(runtimeState?.sessionDisplayId ?? runtimeState?.sessionId) && (
          <PropertyRow label={t("agentDetail.session", { defaultValue: "Session" })}>
        ...
        {runtimeState?.lastError && (
          <PropertyRow label={lastErrorIsActive ? t("taskDetail.lastError") : t("agentDetail.lastRunError", { defaultValue: "Last run error" })}>
            <span
              className={
                lastErrorIsActive
        ...
        {agent.lastHeartbeatAt && (
          <PropertyRow label={t("agentDetail.lastHeartbeat", { defaultValue: "Last Heartbeat" })}>
