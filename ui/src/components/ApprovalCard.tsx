    ...
    <Card className="block border-border/70 p-4">
      <div className="flex items-start justify-between gap-4">
            ...
            <div className="min-w-0 flex-1 space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {requesterAgent && (
                  <div className="inline-flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
                    <span>{t("approvalsPage.requestedBy")}</span>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs leading-5 text-muted-foreground">
                  {t("approvalsPage.requestCreated", { time: timeAgo(approval.createdAt) })}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="shrink-0">
          <div className="inline-flex items-center gap-1.5 rounded-full border border-border/70 bg-background/80 px-2.5 py-1 text-xs text-muted-foreground">
            ...
            <span>{t(`approvalsPage.statuses.${approval.status}`, { defaultValue: approval.status.replace(/_/g, " ") })}</span>
          </div>
        </div>
      </div>
      ...
      {approval.decisionNote && (
        <div className="mt-4 rounded-lg border border-border/60 bg-muted/30 px-3.5 py-3 text-xs leading-5 text-muted-foreground">
          <span className="font-medium text-foreground">{t("approvalsPage.decisionNote")}</span> {approval.decisionNote}
        </div>
      )}
            ...
            {showResolutionButtons && (
              <>
                <Button
                  ...
                  disabled={isPending}
                >
                  {pendingAction === "approve" ? t("approvalsPage.approving") : t("approvalsPage.approve")}
                </Button>
                <Button
                  variant="destructive"
                  size="sm"
                  ...
                  disabled={isPending}
                >
                  {pendingAction === "reject" ? t("approvalsPage.rejecting") : t("approvalsPage.reject")}
                </Button>
              </>
            )}
          </div>
