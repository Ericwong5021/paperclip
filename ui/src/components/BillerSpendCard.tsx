    ...
    <Card>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(row.inputTokens + row.cachedInputTokens)}</span> {t("costsResidual.biller.input")}
              {" · "}
              <span className="font-mono">{formatTokens(row.outputTokens)}</span> {t("costsResidual.biller.output")}
              {" · "}
              {row.providerCount} {t(row.providerCount === 1 ? "costsResidual.biller.providerOne" : "costsResidual.biller.providerMany")}
              {" · "}
              {row.modelCount} {t(row.modelCount === 1 ? "costsResidual.biller.modelOne" : "costsResidual.biller.modelMany")}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        ...
        <div className="text-xs text-muted-foreground">
          {row.apiRunCount > 0
            ? t(row.apiRunCount === 1 ? "costsResidual.biller.meteredRunOne" : "costsResidual.biller.meteredRunMany", { count: row.apiRunCount })
            : t("costsResidual.biller.meteredRunMany", { count: 0 })}
          {" · "}
          {row.subscriptionRunCount > 0
            ? t(row.subscriptionRunCount === 1 ? "costsResidual.biller.subscriptionRunOne" : "costsResidual.biller.subscriptionRunMany", { count: row.subscriptionRunCount })
            : t("costsResidual.biller.subscriptionRunMany", { count: 0 })}
          {" · "}
          {formatCents(weekSpendCents)} {t("costsResidual.biller.thisWeek")}
        </div>
          ...
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("costsResidual.biller.billingTypes")}
              </p>
        ...
        {providerBreakdown.length > 0 && (
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("costsResidual.biller.upstreamProviders")}
              </p>
              <div className="space-y-1.5">
                  ...
                  <div key={entry.provider} className="flex items-center justify-between gap-2 text-xs">
                    ...
                    <div className="text-right tabular-nums">
                      ...
                      <div className="text-muted-foreground">
                        {formatTokens(entry.inputTokens + entry.outputTokens)} {t("costsResidual.biller.tokensShort")}
                      </div>
                    </div>
                  </div>
                ))}
