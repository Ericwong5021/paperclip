...
export function BlockedInboxView({
  ...
  if (error) {
    const message =
      error instanceof Error ? error.message : t("inboxResidual.blocked.loadFailed");
    return (
      <div
      ...
      >
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 space-y-1">
            <p className="text-sm font-medium">{t("inboxResidual.blocked.loadFailed")}</p>
            <p className="text-xs opacity-80">
              {t("inboxResidual.blocked.otherTabsWork", { message })}
            </p>
          </div>
          <Button
            type="button"
            ...
            disabled={isFetching}
          >
            {isFetching ? t("inboxResidual.blocked.trying") : t("inboxResidual.blocked.tryAgain")}
          </Button>
        </div>
      </div>
    );
    ...
    return (
      ...
      >
        ...
        <div className="space-y-1">
          <p className="text-sm font-medium text-foreground">{t("inboxResidual.blocked.empty")}</p>
          <p className="text-xs text-muted-foreground">
            {t("inboxResidual.blocked.emptyHint")}
          </p>
        </div>
      </Card>
    );
  ...
  if (groups.length === 0) {
    return (
      <div className="space-y-3">
        ...
        >
          {t("inboxResidual.blocked.noSearch")}
        </Card>
      </div>
    );
  }
