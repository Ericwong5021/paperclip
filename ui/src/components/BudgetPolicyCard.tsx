...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"]) {
  return windowKind === "lifetime"
    ? "costsResidual.budget.lifetimeBudget"
    : "costsResidual.budget.monthlyUtcBudget";
}

function scopeLabel(scopeType: BudgetPolicySummary["scopeType"]) {
...
export function BudgetPolicyCard({
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("costsResidual.budget.observed")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("costsResidual.budget.ofLimit", { percent: summary.utilizationPercent }) : t("costsResidual.budget.noCapConfigured")}
        </div>
      </div>
      <div>
        <div className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("costsResidual.budget.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("costsResidual.budget.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("costsResidual.budget.softAlertAt", {
            percent: summary.warnPercent,
            pauseSuffix: summary.paused && summary.pauseReason
              ? t("costsResidual.budget.pauseSuffix", { reason: summary.pauseReason })
              : "",
          })}
        </div>
      </div>
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{t("costsResidual.budget.remaining")}</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("costsResidual.budget.unlimited")}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-900 dark:text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("costsResidual.budget.projectPaused")
          : t("costsResidual.budget.scopePaused")}
      </div>
    </div>
  ) : null;
    ...
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      <div className="min-w-0 flex-1">
        <label className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">
          {t("costsResidual.budget.budgetUsd")}
        </label>
        <Input
          value={draftBudget}
      ...
      >
        {isSaving ? t("costsResidual.budget.saving") : summary.amount > 0 ? t("costsResidual.budget.updateBudget") : t("costsResidual.budget.setBudget")}
      </Button>
    </div>
  ) : null;
  ...
  if (isPlain) {
    return (
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          <div>
            <div className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">
              {t(scopeLabel(summary.scopeType))}
            </div>
          ...
          >
            ...
            {summary.paused ? t("costsResidual.budget.paused") : summary.status === "warning" ? t("costsResidual.budget.warning") : summary.status === "hard_stop" ? t("costsResidual.budget.hardStop") : t("costsResidual.budget.healthy")}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("costsResidual.budget.invalidAmount")}</p>
        ) : null}
      </div>
    );
  }
