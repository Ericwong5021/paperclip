...
function detailText(window: QuotaWindow, translate: (key: string, options?: Record<string, unknown>) => string): string | null {
  ...
  return translate("costsResidual.provider.resets", { date: formatted });
}
    ...
    <div className="border border-border px-4 py-4">
      <div className="flex items-start justify-between gap-3 border-b border-border pb-3">
        <div className="min-w-0">
          <div className="text-(length:--text-micro) font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
            {t("costsResidual.provider.codexSubscription")}
          </div>
          <div className="mt-1 text-sm text-muted-foreground">
            {t("costsResidual.provider.liveCodexQuotaWindows")}
          </div>
        </div>
        {source ? (
      ...
      <div className="mt-4 space-y-5">
        <div className="space-y-3">
          <div className="text-(length:--text-micro) font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
            {t("costsResidual.provider.accountWindows")}
          </div>
          <div className="space-y-3">
        ...
        {modelWindows.length > 0 ? (
          <div className="space-y-3">
            <div className="text-(length:--text-micro) font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
            {t("costsResidual.provider.modelWindows")}
            </div>
            <div className="space-y-3">
    ...
    <div className="border border-border px-3.5 py-3">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="shrink-0 text-sm font-semibold tabular-nums text-foreground">
          {window.usedPercent}% {t("costsResidual.provider.used")}
        </div>
      </div>
