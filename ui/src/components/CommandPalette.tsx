...
export function CommandPalette() {
  ...
  return (
    <CommandDialog open={open} onOpenChange={(v) => {
      ...
      <CommandInput
        placeholder={t("commonResidual.commandPalette.placeholder")}
        value={query}
      ...
      <CommandList>
        <CommandEmpty>
          {showSearchAll ? (
            <span>
              {t("commonResidual.commandPalette.noQuickMatches")} {" "}
              <kbd className="rounded border border-border bg-muted px-1 py-0.5 text-(length:--text-nano)">↵</kbd>{" "}
              <span className="font-medium">{t("commonResidual.commandPalette.searchAllAction")}</span>{" "}
              {t("commonResidual.commandPalette.refineHint")}
            </span>
          ) : (
            t("commonResidual.commandPalette.noResults")
          )}
        </CommandEmpty>

        {showSearchAll ? (
          <CommandGroup heading={t("commonResidual.commandPalette.search")}>
            <CommandItem
            ...
            >
              ...
              <span className="flex-1 truncate">
                {t("commonResidual.commandPalette.searchAllFor")} <span className="font-semibold">&ldquo;{searchQuery}&rdquo;</span>
              </span>
              <span className="ml-auto inline-flex items-center gap-1 text-xs text-muted-foreground">
                <span>{t("commonResidual.commandPalette.openFullSearch")}</span>
        ...
        <CommandGroup heading={t("commonResidual.commandPalette.quickFilters")}>
        ...
        {showPromotedProjects && (
          <>
            <CommandGroup heading={t("commonResidual.commandPalette.projects")}>
        ...
        <CommandGroup heading={t("commonResidual.commandPalette.actions")}>
          <CommandItem
            onSelect={() => {
              setOpen(false);
              openNewIssue();
          ...
          >
            ...
            {t("commonResidual.commandPalette.createTask")}
          ...
          {onIssueDetail && fileViewerEnabled && (
            ...
            >
              ...
              {t("commonResidual.commandPalette.openTaskFile")}
          ...
          >
            ...
            {t("commonResidual.commandPalette.createAgent")}
          </CommandItem>
          <CommandItem onSelect={() => go("/projects")}>
            <Plus className="mr-2 h-4 w-4" />
            {t("commonResidual.commandPalette.createProject")}
          </CommandItem>
        </CommandGroup>

        <CommandSeparator />

        <CommandGroup heading={t("commonResidual.commandPalette.pages")}>
          <CommandItem onSelect={() => go("/dashboard")}>
            <LayoutDashboard className="mr-2 h-4 w-4" />
            {t("commonResidual.commandPalette.dashboard")}
          </CommandItem>
          <CommandItem onSelect={() => go("/inbox")}>
            <Inbox className="mr-2 h-4 w-4" />
            {t("commonResidual.commandPalette.inbox")}
          </CommandItem>
          <CommandItem onSelect={() => go("/issues")}>
            <CircleDot className="mr-2 h-4 w-4" />
            {t("commonResidual.commandPalette.tasks")}
          </CommandItem>
          <CommandItem onSelect={() => go("/projects")}>
            <Hexagon className="mr-2 h-4 w-4" />
            {t("commonResidual.commandPalette.projects")}
          </CommandItem>
          <CommandItem onSelect={() => go("/goals")}>
            <Target className="mr-2 h-4 w-4" />
            {t("commonResidual.commandPalette.goals")}
          </CommandItem>
          <CommandItem onSelect={() => go("/agents")}>
            <Bot className="mr-2 h-4 w-4" />
            {t("commonResidual.commandPalette.agents")}
          </CommandItem>
          <CommandItem onSelect={() => go("/costs")}>
            <DollarSign className="mr-2 h-4 w-4" />
            {t("commonResidual.commandPalette.costs")}
          </CommandItem>
          <CommandItem onSelect={() => go("/activity")}>
            <History className="mr-2 h-4 w-4" />
            {t("commonResidual.commandPalette.activity")}
          </CommandItem>
        </CommandGroup>
        ...
        {visibleIssues.length > 0 && (
          <>
            <CommandSeparator />
            <CommandGroup heading={t("commonResidual.commandPalette.tasks")}>
        ...
        {agents.length > 0 && (
          <>
            <CommandSeparator />
            <CommandGroup heading={t("commonResidual.commandPalette.agents")}>
