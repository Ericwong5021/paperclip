  ...
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">{t("costsResidual.finance.recentFinancialEvents")}</CardTitle>
        <CardDescription>{t("costsResidual.finance.recentFinancialEventsDescription")}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3 px-4 pb-4 pt-3">
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">{emptyMessage ?? t("costsResidual.finance.noEvents")}</p>
        ) : (
          rows.map((row) => (
            <div
              key={row.id}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 space-y-2">
                  ...
                  {(row.description || row.externalInvoiceId || row.region || row.pricingTier) && (
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      {row.externalInvoiceId ? <div>{t("costsResidual.finance.invoice", { value: row.externalInvoiceId })}</div> : null}
                      {row.region ? <div>{t("costsResidual.finance.region", { value: row.region })}</div> : null}
                      {row.pricingTier ? <div>{t("costsResidual.finance.tier", { value: row.pricingTier })}</div> : null}
                    </div>
                  )}
                </div>
                <div className="text-right tabular-nums">
                  ...
                  {row.estimated ? <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-amber-600">{t("costsResidual.finance.estimated")}</div> : null}
                </div>
              </div>
            </div>
          ))
