...
function BlockerRecoveryIndicator({ action }: { action: IssueRecoveryAction }) {
  ...
  return (
    <Badge variant="outline"
      ...
      role="status"
      aria-label={label}
      title={t("issueBlocked.recoveryIndicatorTitle", { label })}
...
function SuccessfulRunRetryNowControl({
  ...
  const displayRelative = relative ? localizeRelativeOffset(relative) : null;
  const scheduleLabel = relative === "now"
    ? t("issueBlocked.dueNow")
    : relative
      ? t("issueBlocked.scheduledFor", { relative: displayRelative })
      : t("issueBlocked.scheduled");
    ...
    <div className="mt-2 rounded-md border border-amber-300/70 bg-background/80 p-2 dark:border-amber-500/40 dark:bg-background/40">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0 text-xs leading-5 text-amber-900 dark:text-amber-100">
          {t("issueBlocked.retryDescription", { scheduleLabel })}
        </div>
        <Button
          type="button"
          variant="outline"
        ...
        >
          {retryNow.isPending ? (
            <span className="inline-flex items-center gap-1.5">
              ...
              {t("issueBlocked.retrying")}
            </span>
          ) : success ? (
            <span className="inline-flex items-center gap-1.5">
              <CheckCircle2 className="h-3.5 w-3.5" aria-hidden="true" />
              {retryNow.data?.outcome === "already_promoted"
                ? t("issueBlocked.alreadyPromoted")
                : t("issueBlocked.promoted")}
            </span>
          ) : (
            <span className="inline-flex items-center gap-1.5">
              <RotateCcw className="h-3.5 w-3.5" aria-hidden="true" />
              {t("issueBlocked.retryNow")}
            </span>
          )}
        </Button>
      </div>
...
function localizeRelativeOffset(relative: string): string {
  if (getLocale() !== "zh-CN") return relative;
  ...
  if (relative.startsWith("in ")) {
    return t("issueBlocked.relativeIn", { duration: duration.slice(3).trim() });
  }
  if (relative.endsWith(" ago")) {
    return t("issueBlocked.relativeAgo", { duration: duration.slice(0, -4).trim() });
  }
  return relative;
}
...
function waitingTaskStatusLabel(status: string): string {
  const fallback = status.replace(/_/g, " ").replace(/\b\w/g, (character) => character.toUpperCase());
  return t(`issueBlocked.status.${status}`, { defaultValue: fallback });
}
  ...
  return (
    ...
    >
      <StatusGlyph
        ...
        size="sm"
        title={t("issueBlocked.statusTitle", { status: waitingTaskStatusLabel(blocker.status) })}
      />
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="space-y-1">
            <p className="font-medium leading-5">{t("issueBlocked.waitingLiveTitle")}</p>
            <p className="leading-5">
              {t(total === 1 ? "issueBlocked.queuedBehindSingle" : "issueBlocked.queuedBehindMany", { count: total })}
            </p>
          </div>

          <div className="space-y-1" data-testid="issue-blocked-notice-progress">
            <div className="text-xs font-medium text-blue-800 dark:text-blue-200">
              {t("issueBlocked.progress", { done: doneCount, total })}
              {runningCount > 0 ? ` · ${t("issueBlocked.runningProgress", { count: runningCount })}` : null}
            </div>
            <div
              role="progressbar"
              aria-label={t("issueBlocked.blockerChainProgress")}
              aria-valuemin={0}
            ...
            >
              {steps.map(({ blocker, status }) => (
                <span
                  ...
                  title={t("issueBlocked.progressSegmentTitle", {
                    id: blocker.identifier ?? blocker.id.slice(0, 8),
                    status: waitingTaskStatusLabel(status),
                  })}
                  aria-hidden
                />
              ))}
            ...
            <div className="flex items-stretch gap-2">
              ...
              <div className="min-w-0 pb-0.5">
                <span className="inline-block rounded-md border border-dashed border-blue-300/70 px-2 py-1 text-xs text-blue-800 dark:border-blue-500/40 dark:text-blue-200">
                  {t("issueBlocked.thisTaskResumes")}
                </span>
              </div>
            </div>
          </div>
          ...
          {parkedBlockers.length > 0 ? (
            ...
            >
              <span className="inline-flex items-center gap-1 text-xs font-medium text-amber-800 dark:text-amber-200">
                ...
                {t("issueBlocked.blockedByParkedWork")}
              </span>
...
export function IssueBlockedNotice({
  ...
  const blockerLabel = t(blockers.length === 1 ? "issueBlocked.linkedTask" : "issueBlocked.linkedTasks");
  ...
  const responsibleName = agentName ?? t("issueBlocked.assignee");
  ...
  const reopenSuppressedLeafStatus = reopenSuppressedLeaf
    ? waitingTaskStatusLabel(reopenSuppressedLeaf.status)
    : null;
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 space-y-1.5">
          {showSuccessfulRunHandoff ? (
            <>
              <p className="font-medium leading-5">{t("issueBlocked.successfulRunTitle")}</p>
              <p className="leading-5">{t("issueBlocked.successfulRunBody")}</p>
              <ul className="list-disc space-y-1 pl-5 text-xs leading-5 text-amber-900 dark:text-amber-100">
                <li>{t("issueBlocked.markDoneOrCancelled")}</li>
                <li>{t("issueBlocked.sendForReview")}</li>
                <li>{t("issueBlocked.recordBlocker")}</li>
                <li>{t("issueBlocked.delegateFollowUp")}</li>
              </ul>
              <div className="flex flex-wrap gap-1.5 text-xs">
                {successfulRunHandoff.sourceRunId && successfulRunHandoff.assigneeAgentId ? (
                  ...
                  >
                    {t("issueBlocked.run", { id: successfulRunHandoff.sourceRunId.slice(0, 8) })}
                  </Link>
                ...
                <span className="rounded-md border border-amber-300/70 bg-background/80 px-2 py-1 text-amber-900 dark:border-amber-500/40 dark:bg-background/40 dark:text-amber-100">
                  {t("issueBlocked.askedAssignee", { assignee: agentName ?? t("issueBlocked.assignee") })}
                </span>
              </div>
              {successfulRunHandoff.detectedProgressSummary ? (
                <p className="text-xs leading-5 text-amber-800 dark:text-amber-200">
                  {t("issueBlocked.detectedProgress", { summary: successfulRunHandoff.detectedProgressSummary })}
                </p>
              ) : null}
                ...
                {blockers.length > 0
                  ? isStalled
                    ? stalledLeafBlockers.length > 1
                      ? t("issueBlocked.stalledMultiple", { blockers: blockerLabel })
                      : t("issueBlocked.stalledSingle", { blockers: blockerLabel })
                    : reopenSuppressed
                      ? t(
                          blockers.length === 1
                            ? "issueBlocked.suppressedSingle"
                            : "issueBlocked.suppressedMultiple",
                          { blockers: blockerLabel, assignee: responsibleName },
                        )
                      : t(
                          blockers.length === 1
                            ? "issueBlocked.blockedUntilCompleteSingle"
                            : "issueBlocked.blockedUntilComplete",
                          { blockers: blockerLabel },
                        )
                  : t("issueBlocked.blockedUntilTodo")}
              </p>
              {reopenSuppressed && reopenSuppressedLeafId ? (
                ...
                >
                  ...
                  {reopenSuppressedOtherCount > 0 ? (
                    <> {reopenSuppressedOtherCount} {t(
                      reopenSuppressedOtherCount === 1
                        ? "issueBlocked.otherTask"
                        : "issueBlocked.otherTasks",
                    )}</>
                  ) : null}
                  .
                </p>
              ...
              {showStalledRow ? (
                <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                  <span className="text-xs font-medium text-amber-800 dark:text-amber-200">
                    {t("issueBlocked.stalledInReview")}
                  </span>
                ...
                <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                  <span className="text-xs font-medium text-amber-800 dark:text-amber-200">
                    {t("issueBlocked.ultimatelyWaitingOn")}
                  </span>
