...
export function IssueCasesPanel({ issueId }: { issueId: string }) {
  ...
  return (
    <section className="space-y-2">
      <h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t("workflow.pipelineCasesTraining.cases")}</h3>
      <div className="space-y-1">
        {links.map((link) => (
          ...
          >
            ...
            <Badge variant="secondary" className="shrink-0">{t(`workflow.pipelineCasesTraining.${ROLE_LABEL[link.role]}`)}</Badge>
