  ...
  return (
    <>
      ...
      >
        {saveConflict ? (
          <div className="flex items-center gap-2 text-sm text-amber-800 dark:text-amber-200">
            ...
            <span>{t("routineStatus.saveBar.changedElsewhere")}</span>
          </div>
        ) : (
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                ...
                <span className="font-medium">
                  {t(dirtyCount === 1 ? "routineStatus.saveBar.unsaved" : "routineStatus.saveBar.unsaved_other", { count: dirtyCount })}
                </span>
              </button>
            </PopoverTrigger>
            <PopoverContent align="start" className="w-64">
              <p className="mb-2 text-xs font-medium text-muted-foreground">
                {t("routineStatus.saveBar.pendingChanges")}
              </p>
          ...
          {saveConflict ? (
            <>
              <Button variant="outline" size="sm" onClick={onReload}>
                {t("routineStatus.saveBar.reloadLatest")}
              </Button>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    >
                      ...
                      {t("routineStatus.saveBar.overwrite")}
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>
                    {t("routineStatus.saveBar.overwriteHint")}
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </>
            ...
            <>
              ...
              >
                {t("routineStatus.saveBar.discard")}
              </Button>
              <Button
                size="sm"
                disabled={isSaving || disabled}
              ...
              >
                {isSaving ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : null}
                {t("routineStatus.saveBar.saveChanges")}
      ...
      <Dialog open={confirmDiscardOpen} onOpenChange={setConfirmDiscardOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("routineStatus.saveBar.discardTitle")}</DialogTitle>
            <DialogDescription>
              {t(dirtyCount === 1 ? "routineStatus.saveBar.discardDescription" : "routineStatus.saveBar.discardDescription_other", { count: dirtyCount })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="ghost" size="sm" onClick={() => setConfirmDiscardOpen(false)}>
              {t("routineStatus.saveBar.keepEditing")}
            </Button>
            <Button
              variant="destructive"
              size="sm"
            ...
            >
              {t("routineStatus.saveBar.discardChanges")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
export function RoutineReadOnlyStrip() {
  return (
    <div className="-mx-8 mt-6 border-t border-border bg-muted/20 px-8 py-3 text-xs text-muted-foreground">
      {t("routineStatus.saveBar.readOnly")}
    </div>
  );
}
