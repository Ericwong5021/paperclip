...
export function RoutineTriggerCard({
  ...
  return (
    <form
      aria-label={t("routineStatus.trigger.triggerLabel", { label: trigger.label ?? trigger.kind })}
    ...
    >
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">
            {trigger.kind === "schedule" && trigger.nextRunAt
              ? t("routineStatus.trigger.next", { time: new Date(trigger.nextRunAt).toLocaleString() })
              : trigger.kind === "webhook"
                ? t("routineStatus.trigger.webhook")
                : t("routineStatus.trigger.api")}
          </span>
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="space-y-1.5">
          <Label className="text-xs">{t("routineStatus.trigger.label")}</Label>
          <Input
            value={draft.label}
            disabled={disabled}
        ...
        {trigger.kind === "schedule" && (
          <div className="space-y-1.5 md:col-span-2">
            <Label className="text-xs">{t("routineStatus.trigger.schedule")}</Label>
            <ScheduleEditor
        ...
        {trigger.kind === "webhook" && (
          <>
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routineStatus.trigger.signingMode")}</Label>
              <Select
            ...
            {!SIGNING_MODES_WITHOUT_REPLAY_WINDOW.has(draft.signingMode) && (
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routineStatus.trigger.replayWindow")}</Label>
                <Input
      ...
      {!disabled && (
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          >
            ...
            {t("routineStatus.trigger.delete")}
          </Button>
          {trigger.kind === "webhook" && (
            <Button variant="outline" size="sm" onClick={() => onRotate(trigger.id)}>
              <RefreshCw className="mr-1.5 h-3.5 w-3.5" />
              {t("routineStatus.trigger.rotateSecret")}
            </Button>
          )}
          <Button
            variant="outline"
