  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="overflow-hidden rounded-lg border border-border/70">
      <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
        <div>
          <p className="text-sm font-medium">{translate("routineStatus.variables.variables")}</p>
          <p className="text-xs text-muted-foreground">
            {translate("routineStatus.variables.detected")}
          </p>
        </div>
          ...
          <div key={variable.name} className="p-4">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {translate("routineStatus.variables.promptBeforeRun")}
              </span>
            </div>

            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{translate("routineStatus.variables.label")}</Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">{translate("routineStatus.variables.type")}</Label>
                <Select
                  value={variable.type}
              ...
              <div className="space-y-1.5 md:col-span-2">
                <div className="flex items-center justify-between gap-3">
                  <Label className="text-xs">{translate("routineStatus.variables.defaultValue")}</Label>
                  <label className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    />
                    {translate("routineStatus.variables.required")}
                  </label>
                </div>
                ...
                {variable.type === "textarea" ? (
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="__unset__">{translate("routineStatus.variables.noDefault")}</SelectItem>
                      <SelectItem value="true">{translate("routineStatus.variables.true")}</SelectItem>
                      <SelectItem value="false">{translate("routineStatus.variables.false")}</SelectItem>
                    </SelectContent>
                  </Select>
                ) : variable.type === "select" ? (
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      <Label className="text-xs">{translate("routineStatus.variables.options")}</Label>
                      <Input
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs">{translate("routineStatus.variables.defaultOption")}</Label>
                      <Select
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue placeholder={translate("routineStatus.variables.noDefault")} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="__unset__">{translate("routineStatus.variables.noDefault")}</SelectItem>
                  ...
                  <Input
                    ...
                    placeholder={variable.type === "number" ? "42" : translate("routineStatus.variables.defaultValuePlaceholder")}
                  />
                )}
              </div>
            </div>
...
const BUILTIN_VARIABLE_DOCS: BuiltinVariableDoc[] = [
  {
    name: "date",
    example: "2026-04-28",
    description: t("routineStatus.variables.dateDescription"),
  },
  {
    name: "timestamp",
    example: "April 28, 2026 at 12:17 PM UTC",
    description: t("routineStatus.variables.timestampDescription"),
  },
];
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-2 rounded-lg border border-dashed border-border/70 px-3 py-2 text-xs text-muted-foreground">
        <span>
          {t("routineStatus.variables.usePlaceholder")}
        </span>
        <button
          type="button"
          ...
          aria-label={t("routineStatus.variables.showHelp")}
        >
      ...
      <Dialog open={helpOpen} onOpenChange={setHelpOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("routineStatus.variables.routineVariables")}</DialogTitle>
            <DialogDescription>
              {t("routineStatus.variables.helpDescription")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-5 text-sm">
            <section className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
                {t("routineStatus.variables.custom")}
              </h3>
              <p className="text-muted-foreground">
                {t("routineStatus.variables.typeAnywhere", { variable: "{{variable_name}}" })}{" "}
                <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs text-foreground">
                  {"{{variable_name}}"}
                </code>{" "}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
                <li>{t("routineStatus.variables.customNameRule")}</li>
                <li>{t("routineStatus.variables.customTypeRule")}</li>
                <li>{t("routineStatus.variables.customDateRule")}</li>
                <li>{t("routineStatus.variables.customReuseRule")}</li>
              </ul>
            </section>

            <section className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
                {t("routineStatus.variables.builtIn")}
              </h3>
              <p className="text-muted-foreground">
                {t("routineStatus.variables.builtInDescription")}
              </p>
              <div className="overflow-hidden rounded-lg border border-border/70">
                <table className="w-full text-left text-xs">
                  <thead className="bg-muted/40 text-muted-foreground">
                    <tr>
                      <th className="px-3 py-2 font-medium">{t("routineStatus.variables.placeholder")}</th>
                      <th className="px-3 py-2 font-medium">{t("routineStatus.variables.example")}</th>
                      <th className="px-3 py-2 font-medium">{t("routineStatus.variables.description")}</th>
                    </tr>
                  </thead>
