...
function SidebarAgentItem({
  ...
  const pauseResumeLabel = isPaused ? t("nav.resumeAgent") : t("nav.pauseAgent");
  ...
  const pauseResumeDisabledLabel = disabled
    ...
    : isBudgetPaused
      ...
      : isPaused && hasInvalidOrgChain
        ? t("nav.invalidOrgChain")
      : pauseResumeLabel;
  const showBuiltInLifecycle = builtInStatus === "needs_setup" || builtInStatus === "pending_approval";
  const trailingLabel = [
    showBuiltInLifecycle ? t("settingsShell.sidebar.builtInAgent", { status: builtInStatus.replace(/_/g, " ") }) : null,
    hasInvalidOrgChain ? t("nav.invalidReportingChain") : null,
      ...
      trailing={
        showBuiltInLifecycle || hasInvalidOrgChain ? (
          <span className="ml-1 flex shrink-0 items-center gap-1">
            ...
            {hasInvalidOrgChain ? (
              <AlertTriangle className="h-3.5 w-3.5 shrink-0 text-amber-500" aria-label={t("nav.invalidReportingChain")} />
            ) : null}
          </span>
        ) : undefined
      }
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-48">
          {onToggleStar ? (
            <>
              ...
              >
                ...
                <span>{starred ? t("nav.removeFromStarred") : t("nav.starAgent")}</span>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
            </>
          ) : null}
          ...
          <DropdownMenuItem
            ...
            title={isBudgetPaused ? t("nav.pausedByBudget") : undefined}
          >
          ...
          >
            ...
            <span>{leaving ? t("nav.leaving") : t("nav.leaveAgent")}</span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
...
export function SidebarAgents({ streamlined = false }: { streamlined?: boolean } = {}) {
  ...
  const agentSortChoices: SidebarSectionRadioChoice[] = [
    { value: "top", label: t("nav.sortTop") },
    { value: "alphabetical", label: t("nav.sortAlphabetical") },
    { value: "recent", label: t("nav.sortRecent") },
  ];
  ...
  const pauseResumeAgent = useMutation({
    ...
    onSuccess: async (_agent, { agent, action }) => {
      ...
      pushToast({
        title: action === "pause" ? t("settingsShell.sidebar.agentPaused") : t("settingsShell.sidebar.agentResumed"),
        body: agent.name,
        tone: "success",
      });
    },
    onError: (error, { agent, action }) => {
      pushToast({
        title: action === "pause" ? t("settingsShell.sidebar.pauseAgentFailed") : t("settingsShell.sidebar.resumeAgentFailed"),
