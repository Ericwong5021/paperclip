...
export function StageSecretsPanel({
  ...
  if (!hasAutomation) {
    return (
      <EmptyState
        icon={KeyRound}
        message={t("workflowResidual.stageSecrets.unavailable")}
        action={t("workflowResidual.stageSecrets.setupAutomation")}
        onAction={onSetupAutomation}
      />
    );
  }

  const displayName = agentName?.trim() || t("workflowResidual.stageSecrets.responsibleAgent");
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start gap-2 rounded-md border border-border bg-muted/20 px-4 py-3 text-xs text-muted-foreground">
        ...
        <p>
          {t("workflowResidual.stageSecrets.injectionBefore")} {" "}
          <span className="font-medium text-foreground">{displayName}</span>{" "}
          {t("workflowResidual.stageSecrets.injectionAfter")} <span className="font-mono">PAPERCLIP_*</span>{" "}
          {t("workflowResidual.stageSecrets.reservedNames")}
        </p>
      </div>

      {secretsLoading ? (
        <p className="text-sm text-muted-foreground">{t("workflowResidual.stageSecrets.loading")}</p>
      ) : (
      ...
      <div className="flex items-center gap-3">
        <Button type="button" onClick={onSave} disabled={!dirty || saving}>
          ...
          {saving ? t("workflowResidual.stageSecrets.saving") : t("workflowResidual.stageSecrets.save")}
        </Button>
        {dirty && !saving ? <span className="text-xs text-muted-foreground">{t("workflowResidual.stageSecrets.unsaved")}</span> : null}
      </div>
    </div>
  );
}
