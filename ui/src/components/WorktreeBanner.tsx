  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 overflow-hidden whitespace-nowrap">
        <span className="shrink-0 opacity-70">{t("workflowResidual.worktree.label")}</span>
        <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-current opacity-70" aria-hidden="true" />
        <button
          type="button"
          onClick={handleCopyName}
          title={t("workflowResidual.worktree.copyTitle")}
        ...
        >
          {copied ? t("workflowResidual.worktree.copied") : branding.name}
        </button>
      </div>
    </div>
  );
