...
function bindingRows(binding: ActionCardBinding, isStale: boolean): BindingRow[] {
  const catalogValue = isStale && binding.previousCatalogSha256 ? (
    <span className="inline-flex flex-wrap items-center gap-1.5">
      <span className="text-muted-foreground line-through decoration-amber-500" title={t("interactions.actionCardPreviousCatalogHash")}>
        {shortSha(binding.previousCatalogSha256)}
      </span>
      <span className="text-amber-600 dark:text-amber-400" title={t("interactions.actionCardCurrentCatalogHash")}>
  ...
  return [
    {
      label: t("interactions.actionCardApplication"),
      value: (
        <span>
          {binding.application}
    ...
    { label: t("interactions.actionCardConnection"), value: binding.connection, mono: true },
    { label: t("interactions.actionCardCatalog"), value: catalogValue, mono: !isStale },
    {
      label: t("interactions.actionCardPayload"),
      value: (
        <span className="inline-flex items-center gap-1.5">
          ...
          <span className="font-sans text-(length:--text-micro) uppercase tracking-normal text-muted-foreground">{t("interactions.actionCardSigned")}</span>
        </span>
      ),
      mono: true,
    },
    ...
    >
      <CardContent className="space-y-3 p-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm text-foreground">
              <span className="font-medium">{agentName}</span> {t("interactions.actionCardRequestedApproval")}
            </p>
        ...
        {isStale ? (
          <EnforcementBanner
            tone="warning"
            title={t("interactions.actionCardStaleTitle")}
            body={t("interactions.actionCardStaleBody")}
          />
        ) : null}
        ...
        <div className="space-y-1">
          <p className="text-xs font-medium uppercase tracking-normal text-muted-foreground">{t("interactions.actionCardInput")}</p>
        ...
        {/* Why I'm asking */}
        <div className="space-y-1">
          <p className="text-xs font-medium uppercase tracking-normal text-muted-foreground">{t("interactions.actionCardWhyAsking")}</p>
          <p className="text-sm text-muted-foreground">
            {reason}
            {policyNumber != null ? (
              <>
                {" "}
                {t("interactions.actionCardPolicyRequires", { number: policyNumber })}
              </>
            ) : null}
          </p>
        </div>
      ...
      >
        <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          {expiresInLabel ?? t("interactions.actionCardNoExpiry")}
        </span>
        {mobile ? (
