...
function ImagePreview({ artifact }: { artifact: CompanyArtifact }) {
  ...
  if (errored || !artifact.contentPath) {
    return <PlaceholderPreview label={t("commonResidual.artifacts.types.image")} />;
  }
  return (
    <PreviewFrame>
      <img
...
function TextPreview({ artifact }: { artifact: CompanyArtifact }) {
  ...
  if (!preview) {
    return <PlaceholderPreview label={artifact.source === "document" ? t("commonResidual.artifacts.types.document") : t("commonResidual.artifacts.types.text")} />;
  }
  return (
...
export function ArtifactPreview({ artifact }: { artifact: CompanyArtifact }) {
  ...
  switch (artifact.mediaKind) {
    ...
    case "file":
      return <PlaceholderPreview label={t("commonResidual.artifacts.types.file")} />;
    case "empty":
    default:
      ...
      <div className="flex flex-1 flex-col gap-1 p-3">
        <div className="flex h-7 items-start justify-between gap-2">
          ...
          <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100">
            {artifact.openPath ? (
              <SecondaryAction href={artifact.openPath} title={t("commonResidual.artifacts.openFile")}>
                <ExternalLink className="h-3.5 w-3.5" />
              </SecondaryAction>
            ) : null}
            {artifact.downloadPath ? (
              <SecondaryAction href={artifact.downloadPath} download title={t("commonResidual.artifacts.downloadFile")}>
        ...
        <div className="mt-0.5 flex items-center gap-1.5 text-(length:--text-micro) text-muted-foreground/65">
          <span>{t("commonResidual.artifacts.lastEdited", { date: formatDate(artifact.updatedAt) })}</span>
