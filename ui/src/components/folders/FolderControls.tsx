...
function selectionLabel({
  ...
  if (selection === "unfiled") return t("skills.unfiled");
...
export function FolderRail({
  ...
  return (
    <nav aria-label={`${itemLabelPlural} ${t("skills.folders")}`} className="hidden w-(--sz-folder-rail) shrink-0 border-r border-border pr-3 md:block">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("skills.folders")}</div>
        <Button variant="ghost" size="icon-sm" title={t("skills.newFolder")} onClick={onCreate}>
      ...
      {loading ? (
        ...
        <div className="space-y-0.5">
          ...
          {renderVirtualRow("unfiled", t("skills.unfiled"), result?.unfiledCount ?? 0, <FolderSwatch color={null} className="mt-0.5" />)}
        </div>
      )}
    </nav>
  );
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={t("skills.folderActionsFor", { name: folder.name })}
          >
            <MoreHorizontal className="h-3.5 w-3.5" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end">
          <DropdownMenuItem onSelect={onStartRename}>{t("skills.rename")}</DropdownMenuItem>
          <DropdownMenuItem onSelect={onEdit}>{t("skills.editFolder")}</DropdownMenuItem>
          <DropdownMenuSeparator />
  ...
  return (
    <div className="mb-3 flex flex-wrap items-center gap-2 rounded-md border border-border bg-muted/40 px-3 py-2 text-sm">
      ...
      <span className="min-w-0 flex-1 text-muted-foreground">
        {t("skills.folderSuggestion")}
      </span>
      <Button size="sm" variant="outline" onClick={onCreateFolder}>
        {t("skills.createFirstFolder")}
      </Button>
      <Button size="icon-sm" variant="ghost" aria-label={t("skills.dismissFolderSuggestion")} onClick={dismiss}>
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-(--sz-folder-sheet-max) rounded-t-lg pb-4">
        <SheetHeader className="border-b border-border px-4 py-3">
          <SheetTitle>{itemLabelPlural} {t("skills.folders")}</SheetTitle>
        </SheetHeader>
        <div className="overflow-y-auto px-3">
          ...
          {result?.kind === "skill" ? (
            <>
              {model.my ? renderBranch(model.my, t("skills.mySkills")) : null}
              ...
              {model.projects ? renderBranch(model.projects, t("skills.projects")) : null}
              {model.bundled ? renderBranch(model.bundled, t("skills.bundled")) : null}
            </>
          ) : (
...
export function MoveToMenu({
  ...
  return (
    <DropdownMenuSub>
      <DropdownMenuSubTrigger>{t("skills.moveTo")}</DropdownMenuSubTrigger>
    ...
    <>
        <div className="flex items-center gap-2 px-2 py-1.5">
          ...
          <input
            ...
            placeholder={t("skills.searchFolders")}
        ...
        {visibleFolders.length === 0 ? (
          <div className="px-2 py-2 text-xs text-muted-foreground">{t("skills.noFoldersMatch")}</div>
        ) : null}
        <DropdownMenuSeparator />
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{isEdit ? t("skills.editFolder") : t("skills.createFolder")}</DialogTitle>
          <DialogDescription>
            {kind === "routine" ? t("skills.organizeRoutines") : t("skills.organizeSkills")}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-medium" htmlFor="folder-name">{t("skills.nameField")}</label>
            <Input
              id="folder-name"
              value={name}
          ...
          <div className="space-y-2">
            <div className="text-sm font-medium">{t("skills.color")}</div>
            <div className="flex flex-wrap gap-2">
              {FOLDER_COLORS.map((swatch) => (
                <button
                  key={swatch}
                  type="button"
                  aria-label={t("skills.useFolderColor", { color: swatch })}
                  className={cn(
        ...
        <DialogFooter>
          ...
          <Button onClick={() => onSubmit({ name: name.trim(), color })} disabled={pending || !name.trim()}>
            {pending ? t("skills.saving") : isEdit ? t("skills.save") : t("skills.createFolder")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export function DeleteFolderDialog({
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{t("skills.deleteFolder")}</AlertDialogTitle>
          <AlertDialogDescription>
            {t("skills.deleteFolderDescription", { count: folder?.itemCount ?? 0, itemLabel: itemLabelPlural })}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel disabled={pending}>{t("skills.cancel")}</AlertDialogCancel>
          <AlertDialogAction
          ...
          >
            {pending ? t("skills.deleting") : t("skills.deleteFolder")}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
...
export function BulkBar({
  ...
  return (
    <div className="sticky top-2 z-10 flex flex-wrap items-center gap-2 rounded-md border border-border bg-background/95 px-3 py-2 shadow-sm backdrop-blur">
      <span className="mr-auto text-sm text-muted-foreground">{t("skills.selectedCount", { count: selectedCount })}</span>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button size="sm" variant="outline">{t("skills.moveTo")}</Button>
        </DropdownMenuTrigger>
      ...
      <Button size="sm" variant="ghost" onClick={onClear}>{t("skills.deselectAll")}</Button>
      <Button size="sm" onClick={onDone}>{t("skills.done")}</Button>
    </div>
  );
}
