...
import { displaySkillTag } from "../../lib/company-skill-summary";
  ...
  return (
    <div className="relative hidden h-full shrink-0 md:flex" style={{ width: `${width}px` }}>
      ...
      <div className="mb-2 flex items-center justify-between gap-2 pt-0.5">
        ...
        <Button variant="ghost" size="icon-sm" title={t("skills.newFolder")} onClick={() => onCreateFolder(null)}>
      ...
      {loading ? (
        ...
        <div className="space-y-0.5">
          ...
          <RailHeading label={t("skills.company")} onCreate={() => onCreateFolder(null)} />
          {model.company.length > 0 ? (
            ...
            <div className="px-2 py-1 text-xs text-muted-foreground">{t("skills.noCompanyFolders")}</div>
          )}
          ...
          <div className="flex flex-wrap gap-1.5 px-1">
            ...
            {tags
              ...
              .map((tag) => (
                ...
                >
                  <span className="max-w-32 truncate">{displaySkillTag(tag.slug)}</span>
      ...
      <div
        role="separator"
        aria-label={t("skills.resizeSkillFolders")}
  ...
  return (
    <div>
      ...
      >
        <button
          type="button"
          aria-label={isOpen ? t("skills.collapseFolder") : t("skills.expandFolder")}
          className={cn(
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                aria-label={t("skills.folderActionsFor", { name: label })}
              >
            ...
            <DropdownMenuContent align="end">
              {canNest ? (
                <DropdownMenuItem onSelect={() => onCreateFolder(folder.id)}>
                  ...
                  {t("skills.newSubfolder")}
                </DropdownMenuItem>
              ) : null}
              {editable ? (
                <>
                  <DropdownMenuItem onSelect={() => onStartRename(folder)}>{t("skills.rename")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onEditFolder(folder)}>{t("skills.editFolder")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onMoveFolder(folder, isInMySkills ? "company" : "my")}>
                    <MoveRight className="h-3.5 w-3.5" />
                    {isInMySkills ? t("skills.moveToCompany") : t("skills.moveToMySkills")}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
...
export function FolderBreadcrumb({
  ...
  return (
    <nav aria-label={t("skills.folderPath")} className="flex flex-wrap items-center gap-1 text-sm">
      <button
        type="button"
      ...
      {selection === "unfiled" ? (
        <>
          ...
          <span className="rounded px-1.5 py-0.5 font-medium text-foreground">{t("skills.unfiled")}</span>
        </>
      ) : null}
    ...
    return (
      <div key={folder.id}>
        ...
        >
          ...
          {isCurrent ? <span className="text-xs text-muted-foreground">{t("skills.current")}</span> : null}
          {bundled ? <span className="text-xs text-muted-foreground">{t("skills.readOnly")}</span> : null}
          {isChosen ? <Check className="h-3.5 w-3.5" /> : null}
          {nestable ? (
            <button
              type="button"
              title={t("skills.newFolderInside")}
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="max-h-72 overflow-y-auto rounded-md border border-border p-1">
          ...
          >
            ...
            <span className="min-w-0 flex-1 truncate">{t("skills.unfiled")}</span>
            {currentFolderId == null ? <span className="text-xs text-muted-foreground">{t("skills.current")}</span> : null}
        ...
        <div className="min-h-5 text-xs text-muted-foreground">
          {previewPath ? (
            <span>
              {t("skills.movingTo")} <span className="font-mono text-foreground">{previewPath}</span>
            </span>
          ) : (
            <span>{t("skills.pickDestinationFolder")}</span>
          )}
        </div>
        ...
        <DialogFooter>
          ...
          >
            {pending ? t("skills.moving") : t("skills.moveToHere")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  ...
  return (
    <div className="flex items-center gap-2 py-1" style={{ paddingLeft: `${0.5 + depth * 0.9}rem` }}>
      ...
      <Input
        ...
        placeholder={t("skills.folderName")}
        autoFocus
