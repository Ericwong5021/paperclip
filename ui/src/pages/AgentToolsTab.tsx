    ...
    <section className="rounded-lg border border-border bg-card">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border px-3 py-2.5">
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">
            {t("agentTools.installedDescription", { agent: agentName })}
          </p>
        </div>
      ...
      <div className="space-y-3 p-3">
        <InlineBanner tone="info" compact>
          {t("agentTools.accessExplanation")}
        </InlineBanner>
                ...
                <div key={connection.id} className="space-y-2 px-3 py-3">
                  <label className="flex items-start gap-3">
                    <Checkbox
                      ...
                      aria-label={t("agentTools.installOnAgent", { app: connection.name, agent: agentName })}
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-0.5 block text-xs text-muted-foreground">
                        {installedForAll
                          ? t("agentTools.installedForAllHint")
                          : checked
                            ? t("agentTools.loadedIntoContext")
                            : t("agentTools.installHint")}
                      </span>
                    </span>
                  </label>
                  {permitted && !checked ? (
...
function InstallBadge({
  ...
  const label = installed
    ? installedForAll
      ? t("agentTools.installedForAll")
      : t("agentTools.installed")
    : permitted
...
function InstallSaveStatusChip({
  ...
  if (error) return <span className="text-xs text-destructive">{t("agentTools.saveFailed")}</span>;
...
const POLICY_EFFECT_LABEL_KEY: Record<string, string> = {
  allow: "agentTools.policyEffects.allow",
  block: "agentTools.policyEffects.block",
  deny: "agentTools.policyEffects.deny",
  require_approval: "agentTools.policyEffects.requireApproval",
  redact: "agentTools.policyEffects.redact",
  rate_limit: "agentTools.policyEffects.rateLimit",
};
...
export function AgentToolsTab({ agent, companyId }: { agent: AgentDetailRecord; companyId: string }) {
  ...
  if (effective.isLoading) return <ToolsLoadingState label={t("agentTools.resolvingAccess")} />;
  if (effective.error) {
  ...
  return (
    <div className="space-y-4">
      <EnforcementBanner
        ...
        body={t("agentTools.effectiveDescription", { agent: agent.name })}
      />
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-left text-xs text-muted-foreground">
                    <th className="px-3 py-2 font-medium">{t("agentTools.columns.tool")}</th>
                    <th className="px-3 py-2 font-medium">{t("agentTools.columns.capability")}</th>
                    <th className="px-3 py-2 font-medium">{t("agentTools.columns.risk")}</th>
                    <th className="px-3 py-2 font-medium">{t("agentTools.columns.source")}</th>
                  </tr>
                </thead>
                  ...
                  {allowedTools.map((tool) => (
                    <tr key={tool.id} className="align-top">
                      ...
                      <td className="px-3 py-2 text-xs text-muted-foreground">
                        {connectionNameById.get(tool.connectionId) ?? t("agentTools.notAvailable")}
                      </td>
                    </tr>
                  ))}
                </tbody>
                  ...
                  <div key={policy.id} className="rounded-md border border-border/70 px-2.5 py-2">
                    <div className="flex items-center justify-between gap-2">
                      <Link
                        ...
                        title={t("agentTools.policyTitle", { order, name: policy.name })}
                      >
                        #{order} {policy.name}
                      </Link>
              ...
              {catalogLoading ? (
                ...
                <>
                  ...
                  {deniedTools.length > DENIED_TOOLS_DISPLAY_LIMIT ? (
                    <p className="text-(length:--text-micro) text-muted-foreground">
                      {t("agentTools.more", { count: deniedTools.length - DENIED_TOOLS_DISPLAY_LIMIT })}
                    </p>
                  ) : null}
                </>
              )}
