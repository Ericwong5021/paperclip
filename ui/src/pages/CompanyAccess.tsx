...
export function CompanyAccess() {
  ...
  const updateMemberMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({
        title: t("adminResidual.companyAccess.memberUpdated"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("adminResidual.companyAccess.updateMemberFailed"),
  ...
  const approveJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("adminResidual.companyAccess.joinRequestApproved"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("adminResidual.companyAccess.approveJoinRequestFailed"),
        body: error instanceof Error ? error.message : t("admin.common.unknownError"),
        tone: "error",
      });
    },
  ...
  const rejectJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("adminResidual.companyAccess.joinRequestRejected"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("adminResidual.companyAccess.rejectJoinRequestFailed"),
        body: error instanceof Error ? error.message : t("admin.common.unknownError"),
        tone: "error",
      });
    },
  ...
  const archiveMemberMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        title: t("adminResidual.companyAccess.memberRemoved"),
        body:
          result.reassignedIssueCount > 0
            ? t(result.reassignedIssueCount === 1
              ? "adminResidual.companyAccess.assignedTaskCleanedUp"
              : "adminResidual.companyAccess.assignedTasksCleanedUp", { count: result.reassignedIssueCount })
            : undefined,
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("adminResidual.companyAccess.removeMemberFailed"),
        body: error instanceof Error ? error.message : t("admin.common.unknownError"),
        tone: "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("adminResidual.companyAccess.selectCompany")}</div>;
  }

  if (membersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("adminResidual.companyAccess.loading")}</div>;
  }

  if (membersQuery.error) {
    const message =
      membersQuery.error instanceof ApiError && membersQuery.error.status === 403
        ? t("adminResidual.companyAccess.noPermission")
        : membersQuery.error instanceof Error
          ? membersQuery.error.message
          : t("adminResidual.companyAccess.loadFailed");
    ...
    <div className="max-w-6xl space-y-8">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("membersPage.title")}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("membersPage.description", { company: selectedCompany?.name })}
        </p>
        <div className="rounded-lg border border-border bg-muted/30 px-4 py-3 text-sm text-muted-foreground">
          {t("membersPage.coreDescription")}
        </div>
      </div>

      {access && !access.currentUserRole && (
        <div className="rounded-xl border border-amber-500/40 px-4 py-3 text-sm text-amber-800 dark:text-amber-200">
          {t("adminResidual.companyAccess.instanceAdminMembershipWarning")}
        </div>
      )}
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{t("membersPage.humans")}</h2>
          </div>
          <p className="max-w-3xl text-sm text-muted-foreground">
            {t("membersPage.humansDescription")}
          </p>
        </div>
        ...
        {access?.canApproveJoinRequests && pendingHumanJoinRequests.length > 0 ? (
          <div className="space-y-3 rounded-xl border border-border px-4 py-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h3 className="text-sm font-semibold">{t("membersPage.pendingJoins")}</h3>
                <p className="text-sm text-muted-foreground">
                  {t("membersPage.pendingJoinsDescription")}
                </p>
              </div>
              <Badge variant="outline">{pendingHumanJoinRequests.length} {t("membersPage.pending")}</Badge>
            </div>
            <div className="space-y-3">
              {pendingHumanJoinRequests.map((request) => (
                <PendingJoinRequestCard
                  ...
                  title={
                    ...
                    t("adminResidual.companyAccess.unknownHumanRequester")
                  }
                  subtitle={
                    request.requesterUser?.email ||
                    request.requestEmailSnapshot ||
                    request.requestingUserId ||
                    t("membersPage.noEmail")
                  }
                  context={
                    request.invite
                      ? `${t("adminResidual.companyAccess.joinInvite", { types: request.invite.allowedJoinTypes })}${request.invite.humanRole ? ` • ${t("adminResidual.companyAccess.defaultRole", { role: request.invite.humanRole })}` : ""}`
                      : t("adminResidual.companyAccess.inviteMetadataUnavailable")
                  }
                  detail={t("adminResidual.companyAccess.submitted", { date: new Date(request.createdAt).toLocaleString() })}
                  approveLabel={t("adminResidual.companyAccess.approveHuman")}
                  rejectLabel={t("adminResidual.companyAccess.rejectHuman")}
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          <div className="grid grid-cols-(--gtc-24) gap-3 border-b border-border px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">
            <div>{t("membersPage.userAccount")}</div>
            <div>{t("membersPage.role")}</div>
            <div>{t("membersPage.status")}</div>
            <div className="text-right">{t("membersPage.action")}</div>
          </div>
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">{t("membersPage.empty")}</div>
          ) : (
            members.map((member) => {
              const removalReason = member.removal?.reason === "You cannot remove yourself."
                ? t("membersPage.cannotRemoveSelf")
                : member.removal?.reason ?? null;
                ...
                >
                  ...
                  <div className="text-sm">
                    {member.membershipRole
                      ? t(`membersPage.${member.membershipRole}`)
                      : t("membersPage.unset")}
                  </div>
                  <div>
                    <Badge variant={member.status === "active" ? "secondary" : member.status === "suspended" ? "destructive" : "outline"}>
                      {member.status === "active" ? t("membersPage.active") : member.status === "pending" ? t("membersPage.pending") : member.status === "suspended" ? t("membersPage.suspended") : member.status.replace("_", " ")}
                    </Badge>
                  </div>
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("adminResidual.companyAccess.editMemberDescription", {
            ...
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("membersPage.companyRole")}</span>
                  <select
                  ...
                  >
                    <option value="">{t("membersPage.unset")}</option>
                    {Object.entries(HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS).map(([value]) => (
                ...
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("membersPage.membershipStatus")}</span>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2"
                    value={draftStatus}
                    onChange={(event) =>
                  ...
                  >
                    <option value="active">{t("membersPage.active")}</option>
                    <option value="pending">{t("membersPage.pending")}</option>
                    <option value="suspended">{t("membersPage.suspended")}</option>
                  </select>
                </label>
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            >
              {updateMemberMutation.isPending ? t("membersPage.saving") : t("membersPage.saveMember")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!removingMember} onOpenChange={(open) => !open && setRemovingMemberId(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("membersPage.removeMember")}</DialogTitle>
            <DialogDescription>
              {t("adminResidual.companyAccess.removeMemberDescription", { name: memberDisplayName(removingMember, t("adminResidual.companyAccess.thisMember")) })}
            </DialogDescription>
          </DialogHeader>
          {removingMember && (
            <div className="space-y-5">
              <div className="rounded-lg border border-border px-3 py-3">
                <div className="text-sm font-medium">{memberDisplayName(removingMember, t("adminResidual.companyAccess.thisMember"))}</div>
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ? t("membersPage.checkingTasks")
                    : t("membersPage.openTasks", { count: assignedIssues.length })}
                </div>
              </div>

              {assignedIssues.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-sm font-medium">{t("adminResidual.companyAccess.taskReassignment")}</div>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                    value={reassignmentTarget}
                    onChange={(event) => setReassignmentTarget(event.target.value)}
                  >
                    <option value="__unassigned">{t("adminResidual.companyAccess.leaveUnassigned")}</option>
                    {activeReassignmentUsers.length > 0 ? (
                      <optgroup label={t("adminResidual.companyAccess.humans")}>
                    ...
                    {activeReassignmentAgents.length > 0 ? (
                      <optgroup label={t("adminResidual.companyAccess.agents")}>
                  ...
                  <div className="max-h-36 overflow-auto rounded-lg border border-border">
                    ...
                    {assignedIssues.length > 6 ? (
                      <div className="px-3 py-2 text-sm text-muted-foreground">
                        {t(assignedIssues.length - 6 === 1
                          ? "adminResidual.companyAccess.moreTask"
                          : "adminResidual.companyAccess.moreTasks", { count: assignedIssues.length - 6 })}
                      </div>
                    ) : null}
                  </div>
                </div>
          ...
          <DialogFooter>
            ...
            >
              {archiveMemberMutation.isPending ? t("membersPage.removing") : t("membersPage.removeMember")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
export function CompanyAccessLegacyRoute() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("settings.title"), href: "/company/settings" },
      { label: t("adminResidual.companyAccess.access") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  if (isLoading) {
    return <div className="text-sm text-muted-foreground">{t("adminResidual.companyAccess.checkingAdvancedPermissions")}</div>;
  }
    ...
    <div className="max-w-2xl space-y-5">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("adminResidual.companyAccess.advancedPermissions")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("adminResidual.companyAccess.advancedPermissionsDescription")}
        </p>
      </div>

      <div className="space-y-4 rounded-xl border border-border px-5 py-5">
        <div className="space-y-2">
          <h2 className="text-sm font-semibold">{t("adminResidual.companyAccess.advancedPermissionsUnavailable")}</h2>
          <p className="text-sm text-muted-foreground">
            {t("adminResidual.companyAccess.advancedPermissionsUnavailableDescription")}
          </p>
          {errorMessage ? (
            <p className="text-sm text-destructive">{t("adminResidual.companyAccess.pluginExtensionsUnavailable", { message: errorMessage })}</p>
          ) : null}
        </div>
        <div className="flex flex-wrap gap-2">
          <Button asChild>
            <Link to="/company/settings/members">{t("membersPage.openMembers")}</Link>
          </Button>
          <Button asChild variant="outline">
            <Link to="/company/settings/invites">{t("adminResidual.companyAccess.openInvites")}</Link>
          </Button>
        </div>
      </div>
    </div>
