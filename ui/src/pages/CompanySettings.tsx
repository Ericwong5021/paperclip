  ...
  if (!selectedCompany) {
    return (
      <div className="text-sm text-muted-foreground">
        {t("adminResidual.companySettings.noCompanySelected")}
      </div>
    );
  }
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          ...
          <Field
            ...
            hint={t("adminResidual.companySettings.descriptionHint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 space-y-3">
              <Field
                ...
                hint={t("adminResidual.companySettings.logoHint")}
              >
                <div className="space-y-2">
                  ...
                  {logoUploadMutation.isPending && (
                    <span className="text-xs text-muted-foreground">{t("adminResidual.companySettings.uploadingLogo")}</span>
                  )}
                </div>
              </Field>
              <Field
                label={t("settings.brandColor")}
                hint={t("adminResidual.companySettings.brandColorHint")}
              >
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    placeholder={t("adminResidual.companySettings.auto")}
                  ...
                  {brandColor && (
                    ...
                    >
                      {t("adminResidual.companySettings.clear")}
                    </Button>
                  )}
                </div>
              </Field>
              <Field
                ...
                hint={t("adminResidual.companySettings.attachmentRange", { max: MAX_COMPANY_ATTACHMENT_MAX_MIB })}
              >
                <div className="flex flex-col gap-1.5">
                  ...
                  {!attachmentMaxValid && (
                    <span className="text-xs text-destructive">
                      {t("adminResidual.companySettings.attachmentWholeNumber", { max: MAX_COMPANY_ATTACHMENT_MAX_MIB })}
                    </span>
                  )}
                </div>
              </Field>
      ...
      {generalDirty && (
        <div className="flex items-center gap-2">
          ...
          {generalMutation.isSuccess && (
            <span className="text-xs text-muted-foreground">{t("adminResidual.companySettings.saved")}</span>
          )}
          {generalMutation.isError && (
            <span className="text-xs text-destructive">
              {generalMutation.error instanceof Error
                  ? generalMutation.error.message
                  : t("adminResidual.companySettings.saveFailed")}
            </span>
          )}
        </div>
      )}
      ...
      <div className="space-y-4" data-testid="company-settings-team-section">
        ...
        <div className="rounded-md border border-border px-4 py-3">
          <ToggleField
            ...
            hint={t("adminResidual.companySettings.hiringHint")}
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() => {
                ...
                const confirmed = window.confirm(
                  t("adminResidual.companySettings.archiveConfirm", { name: selectedCompany.name })
                );
                if (!confirmed) return;
                const nextCompanyId =
                  companies.find(
            ...
            >
              {archiveMutation.isPending
                ...
                : selectedCompany.status === "archived"
                ? t("adminResidual.companySettings.alreadyArchived")
            ...
            {archiveMutation.isError && (
              <span className="text-xs text-destructive">
                {archiveMutation.error instanceof Error
                  ...
                  : t("adminResidual.companySettings.archiveFailed")}
              </span>
            )}
          </div>
        </div>
