  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("costsPage.financeLedger")}</CardTitle>
        <CardDescription>
          {t("costsPage.financeLedgerDescription")}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("costsPage.debits")}
          value={formatCents(debitCents)}
          subtitle={t("costsPage.totalEvents", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("costsPage.credits")}
          value={formatCents(creditCents)}
          subtitle={t("costsPage.creditDescription")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costsPage.net")}
          value={formatCents(netCents)}
          subtitle={t("costsPage.netDescription")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("costsPage.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("costsPage.estimatedDescription")}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("costsPage.title") }]);
  }, [setBreadcrumbs, t]);
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costsPage.allProviders")}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costsPage.allBillers")}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("costsPage.selectCompany")} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("costsPage.title")}</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("costsPage.description")}
                </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {PRESET_KEYS.map((key) => (
                ...
                >
                  {t(`costsPage.${{ mtd: "mtd", "7d": "last7", "30d": "last30", ytd: "ytd", all: "allTime", custom: "custom" }[key]}`)}
                </Button>
              ))}
            </div>
          </div>
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("costsPage.to")}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("costsPage.inferenceSpend")}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("costsPage.tokensAcrossEvents", { tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("costsPage.budget")}
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ...
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ...
                    : t("costsPage.noMonthlyCap")
              }
              icon={Coins}
            />
            <MetricTile
              label={t("costsPage.financeNet")}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("costsPage.debitCredit", {
            ...
            />
            <MetricTile
              label={t("costsPage.financeEvents")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("costsPage.estimatedRange", { amount: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{t("costsPage.overview")}</TabsTrigger>
          <TabsTrigger value="budgets">{t("costsPage.budgets")}</TabsTrigger>
          <TabsTrigger value="providers">{t("costsPage.providers")}</TabsTrigger>
          <TabsTrigger value="billers">{t("costsPage.billers")}</TabsTrigger>
          <TabsTrigger value="finance">{t("costsPage.finance")}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costsPage.selectDates")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-31)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costsPage.inferenceLedger")}</CardTitle>
                    <CardDescription>
                      {t("costsPage.inferenceLedgerDescription")}
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costsPage.budgetAmount", { amount: formatCents(spendData.summary.budgetCents) })
                            : t("costsPage.unlimitedBudget")}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("costsPage.usage")}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("costsPage.monthlyBudgetUsage", { percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-32)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costsPage.byAgent")}</CardTitle>
                    <CardDescription>{t("costsPage.byAgentDescription")}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costsPage.noCostEvents")}</p>
                    ) : (
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("costsPage.apiRuns", { count: row.apiRunCount })}
                                    {" · "}
                                    {t("costsPage.subscriptionRuns", { count: row.subscriptionRunCount })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costsPage.byProject")}</CardTitle>
                      <CardDescription>{t("costsPage.byProjectDescription")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costsPage.noProjectCosts")}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? t("costsPage.unattributed")}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costsPage.noFinanceEventsDetailed")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-(image:--gradient-extract-2)">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("costsPage.budgetControlPlane")}</CardTitle>
                  <CardDescription>{t("costsPage.budgetControlPlaneDescription")}</CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("costsPage.activeIncidents")}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("costsPage.activeIncidentsSubtitle")}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("costsPage.pendingApprovals")}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("costsPage.pendingApprovalsSubtitle")}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("costsPage.pausedAgents")}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("costsPage.pausedAgentsSubtitle")}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("costsPage.pausedProjects")}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("costsPage.pausedProjectsSubtitle")}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costsPage.activeIncidents")}</h2>
                    <p className="text-sm text-muted-foreground">{t("costsPage.activeIncidentsDescription")}</p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                    ...
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold">
                          {t(`costsPage.${scopeType === "company" ? "companyBudgets" : scopeType === "agent" ? "agentBudgets" : "projectBudgets"}`)}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costsPage.companyBudgetPolicyDescription")
                            : scopeType === "agent"
                              ? t("costsPage.agentBudgetPolicyDescription")
                              : t("costsPage.projectBudgetPolicyDescription")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costsPage.noCostEventsInPeriod")}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costsPage.noBillableEventsInPeriod")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-33)">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costsPage.byBiller")}</CardTitle>
                      <CardDescription>{t("costsPage.byBillerDescription")}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costsPage.noFinanceEvents")}</p>
                      ) : (
