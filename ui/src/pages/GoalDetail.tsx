...
export function GoalPropertiesToggleButton({
  ...
  return (
    <Button
      ...
      title={t("goalsPage.showProperties")}
    >
...
export function GoalDetail() {
  ...
  const uploadImage = useMutation({
    mutationFn: async (file: File) => {
      if (!resolvedCompanyId) throw new Error(t("goalsPage.noCompanySelected"));
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("goalsPage.title"), href: "/goals" },
      { label: goal?.title ?? goalId ?? t("goalsPage.goal") }
    ]);
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <span className="text-xs uppercase text-muted-foreground">
            {t(`goalsPage.values.${goal.level}`, { defaultValue: goal.level })}
          </span>
        ...
        <InlineEditor
          ...
          placeholder={t("goalsPage.addDescription")}
          multiline
      ...
      <Tabs defaultValue="children">
        <TabsList>
          <TabsTrigger value="children">
            {t("goalsPage.subGoalsCount", { count: childGoals.length })}
          </TabsTrigger>
          <TabsTrigger value="projects">
            {t("goalsPage.projectsCount", { count: linkedProjects.length })}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="children" className="mt-4 space-y-3">
          ...
          {childGoals.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("goalsPage.noSubGoals")}</p>
          ) : (
        ...
        <TabsContent value="projects" className="mt-4">
          {linkedProjects.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("goalsPage.noLinkedProjects")}</p>
          ) : (
