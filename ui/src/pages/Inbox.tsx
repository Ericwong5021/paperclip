...
function runFailureMessage(run: HeartbeatRun): string {
  return firstNonEmptyLine(run.error) ?? firstNonEmptyLine(run.stderrExcerpt) ?? t("inboxPage.failedRuns");
}
...
export function formatJoinRequestInboxLabel(
  ...
  if (joinRequest.requestType !== "human") {
    return `${t("inboxResidual.ui.agentJoinRequest")}${joinRequest.agentName ? `: ${joinRequest.agentName}` : ""}`;
  }
  ...
  return t("inboxResidual.ui.humanJoinRequest");
}
      ...
      <div className="flex items-start gap-2 sm:items-center">
          ...
          <span className="hidden sm:inline-flex h-4 w-4 shrink-0 items-center justify-center self-center">
            {showUnreadDot ? (
              ...
              <button
                ...
                aria-label={t("inboxResidual.ui.dismissFromInbox")}
              >
          ...
          <span className="min-w-0 flex-1">
            <span className="line-clamp-2 text-sm font-medium sm:truncate sm:line-clamp-none">
              {issue ? (
                ...
                <>{t("inboxPage.failedRuns")}{linkedAgentName ? ` · ${linkedAgentName}` : ""}</>
              )}
            </span>
        ...
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
          ...
          >
            ...
            {isRetrying ? t("inboxPage.retrying") : t("inboxPage.retry")}
          </Button>
          {!showUnreadSlot && (
            <button
              type="button"
              ...
              aria-label={t("inboxResidual.ui.dismiss")}
            >
              <X className="h-4 w-4" />
            </button>
          )}
...
export function Inbox() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("inboxPage.title") }]);
  }, [setBreadcrumbs, t]);
  ...
  const creatorOptions = useMemo<CreatorOption[]>(() => {
    ...
    if (currentUserId) {
      options.set(`user:${currentUserId}`, {
        ...
        label: currentUserId === "local-board" ? t("inboxResidual.ui.board") : t("inboxResidual.ui.me"),
        kind: "user",
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inboxResidual.ui.approveFailed"));
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inboxResidual.ui.rejectFailed"));
    },
  });
  ...
  const approveJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inboxResidual.ui.approveJoinFailed"));
    },
  });
  ...
  const rejectJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inboxResidual.ui.rejectJoinFailed"));
    },
  });
  ...
  const retryRunMutation = useMutation({
    mutationFn: async (run: HeartbeatRun) => {
      ...
      if (!("id" in result)) {
        throw new Error(result.message ?? t("inboxResidual.ui.retrySkipped"));
      }
  ...
  const archiveIssueMutation = useMutation({
    ...
    onError: (err, id, context) => {
      setActionError(err instanceof Error ? err.message : t("inboxResidual.ui.archiveFailed"));
  ...
  const unarchiveIssueMutation = useMutation({
    ...
    onError: (err, id, context) => {
      setActionError(err instanceof Error ? err.message : t("inboxResidual.ui.undoArchiveFailed"));
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={InboxIcon} message={t("inboxPage.selectCompany")} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <div className="relative sm:hidden">
          ...
          <Input
            type="search"
            placeholder={t("inboxPage.search")}
            value={searchQuery}
        ...
        <Tabs value={tab} onValueChange={(value) => navigate(`/inbox/${value}`)}>
          <PageTabBar
            items={[
              {
                value: "mine",
                label: t("inboxPage.mine"),
              },
              {
                value: "recent",
                label: t("inboxPage.recent"),
              },
              { value: "unread", label: t("inboxPage.unread") },
              { value: "blocked", label: t("inboxPage.blocked") },
              { value: "all", label: t("inboxPage.all") },
            ]}
          />
        </Tabs>
          ...
          {tab === "blocked" ? (
            <>
                ...
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="space-y-0.5 p-2">
                    {BLOCKED_GROUP_OPTIONS.map(([value, label]) => (
                      ...
                      >
                        <span>{value === "none" ? t("inboxPage.none") : label}</span>
              ...
              <IssueColumnPicker
                ...
                title={t("inboxPage.chooseColumns")}
                iconOnly
              />
              <Popover>
                <PopoverTrigger asChild>
            ...
            <>
              <Button
                ...
                title={nestingEnabled ? t("inboxPage.disableNesting") : t("inboxPage.enableNesting")}
              >
              ...
              <Popover>
                ...
                <PopoverContent align="end" className="w-40 p-2">
                  <div className="space-y-0.5">
                    {([
                      ["none", t("inboxPage.none")],
                      ["type", t("inboxPage.type")],
                      ["assignee", t("inboxPage.responsible")],
                      ["project", t("inboxPage.project")],
                      ...(isolatedWorkspacesEnabled ? ([["workspace", t("inboxPage.workspace")]] as const) : []),
              ...
              {canMarkAllRead && (
                <>
                  ...
                  >
                    {markAllReadMutation.isPending ? t("inboxPage.marking") : t("inboxPage.markAllRead")}
                  </Button>
                  <Dialog open={showMarkAllReadConfirm} onOpenChange={setShowMarkAllReadConfirm}>
                    <DialogContent className="sm:max-w-md">
                      <DialogHeader>
                        <DialogTitle>{t("inboxPage.markAllReadQuestion")}</DialogTitle>
                        <DialogDescription>
                          {t("inboxPage.markAllReadDescription", { count: unreadIssueIds.length })}
                        </DialogDescription>
                      </DialogHeader>
                      <DialogFooter>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SelectTrigger className="h-8 w-(--sz-170px) text-xs">
              <SelectValue placeholder={t("inboxPage.category")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="everything">{t("inboxPage.allCategories")}</SelectItem>
              <SelectItem value="issues_i_touched">{t("inboxPage.myRecentTasks")}</SelectItem>
              <SelectItem value="join_requests">{t("inboxPage.joinRequests")}</SelectItem>
              <SelectItem value="approvals">{t("inboxPage.approvals")}</SelectItem>
              <SelectItem value="failed_runs">{t("inboxPage.failedRuns")}</SelectItem>
              <SelectItem value="alerts">{t("inboxPage.alerts")}</SelectItem>
            </SelectContent>
          </Select>
          ...
          {showApprovalsCategory && (
            ...
            >
              <SelectTrigger className="h-8 w-(--sz-170px) text-xs">
                <SelectValue placeholder={t("inboxPage.approvalStatus")} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t("inboxPage.allApprovalStatuses")}</SelectItem>
                <SelectItem value="actionable">{t("inboxPage.needsAction")}</SelectItem>
                <SelectItem value="resolved">{t("inboxPage.resolved")}</SelectItem>
              </SelectContent>
            </Select>
          )}
        </div>
      ...
      {tab !== "blocked" && allLoaded && visibleSections.length === 0 && (
        <EmptyState
          ...
          message={
            searchQuery.trim()
              ? t("inboxPage.noSearch")
              : tab === "mine"
              ? t("inboxPage.zero")
              : tab === "unread"
              ? t("inboxPage.noNew")
              : tab === "recent"
                ? t("inboxPage.noRecent")
                : t("inboxPage.noFilters")
          }
        />
      )}
                    ...
                    elements.push(
                      ...
                      >
                        ...
                        <span className="shrink-0 text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
                          {group.searchSection === "archived" ? t("inboxPage.archived") : t("inboxPage.otherResults")}
                        </span>
                        <div className="h-px flex-1 bg-border/80" />
                      </div>,
                    );
                        ...
                        <IssueGroupHeader
                          ...
                          trailing={canCreateIssueInGroup ? (
                            <Button
                              ...
                              title={t("inboxResidual.ui.newTaskIn", { group: group.label })}
                              aria-label={t("inboxResidual.ui.newTaskIn", { group: group.label })}
                              onClick={(event) => {
                                event.stopPropagation();
