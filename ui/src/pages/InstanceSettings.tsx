...
export function InstanceSettings() {
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("instanceSettings.heartbeats.unknownError");
        throw new Error(
          failures.length === 1
            ? t("instanceSettings.heartbeats.disableOneError", { detail })
            : t("instanceSettings.heartbeats.disableManyError", { failed: failures.length, enabled: enabled.length, detail }),
        );
      }
      return enabled;
    },
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instanceSettings.heartbeats.title")} {t("instanceSettings.common.loading").toLowerCase()}</div>;
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceSettings.heartbeats.title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("instanceSettings.heartbeats.description")}
        </p>
      </div>
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        ...
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("instanceSettings.heartbeats.company") : t("instanceSettings.heartbeats.companies")}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              const noun = enabledCount === 1 ? t("instanceSettings.heartbeats.agent") : t("instanceSettings.heartbeats.agents");
              if (!window.confirm(t("instanceSettings.heartbeats.disableAllConfirm", { count: enabledCount, noun }))) {
                return;
              }
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instanceSettings.heartbeats.empty")}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("instanceSettings.heartbeats.on") : t("instanceSettings.heartbeats.off")}
                        </Badge>
                        <Link
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("instanceSettings.heartbeats.disableTimer") : t("instanceSettings.heartbeats.enableTimer")}
                          </Button>
                        </span>
                      </div>
                    );
