...
function formatMs(value: number | null, pendingLabel: string) {
    ...
    <div data-testid="issue-chat-long-thread-perf" className="space-y-5">
      <div className="flex flex-col gap-3 border-b border-border pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="secondary">{t(`issueChatLab.longThread.status.${fixture.issue.status}`, { defaultValue: fixture.issue.status.replace(/_/g, " ") })}</Badge>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
            {t("issueChatLab.longThread.description")}
          </p>
        </div>
        <div className="grid min-w-(--sz-280px) grid-cols-2 gap-2">
          <MetricTile label={t("issueChatLab.longThread.fixtureRows")} value={String(rowTarget)} testId="perf-fixture-row-target" />
          <MetricTile label={t("issueChatLab.longThread.markdownRows")} value={String(LONG_THREAD_MARKDOWN_COMMENT_COUNT)} testId="perf-fixture-markdown-rows" />
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-(--gtc-40)">
        <main className="min-w-0 space-y-4">
          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("issueChatLab.longThread.issueDocuments")}</CardTitle>
            </CardHeader>
          ...
          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("issueChatLab.longThread.subIssues")}</CardTitle>
            </CardHeader>
        ...
        <aside className="space-y-4 xl:sticky xl:top-4 xl:self-start">
          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("issueChatLab.longThread.baselineMetrics")}</CardTitle>
            </CardHeader>
            <CardContent className="grid gap-2">
              <MetricTile label={t("issueChatLab.longThread.profilerCommits")} value={String(metrics.commitCount)} testId="perf-commit-count" />
              <MetricTile label={t("issueChatLab.longThread.mountDuration")} value={formatMs(metrics.mountActualDuration, t("issueChatLab.longThread.pending"))} testId="perf-mount-duration" />
              <MetricTile label={t("issueChatLab.longThread.latestDuration")} value={formatMs(metrics.latestActualDuration, t("issueChatLab.longThread.pending"))} testId="perf-latest-duration" />
              <MetricTile label={t("issueChatLab.longThread.maxDuration")} value={formatMs(metrics.maxActualDuration, t("issueChatLab.longThread.pending"))} testId="perf-max-duration" />
              <MetricTile label={t("issueChatLab.longThread.totalDuration")} value={formatMs(metrics.totalActualDuration, t("issueChatLab.longThread.pending"))} testId="perf-total-duration" />
            </CardContent>
          </Card>

          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("issueChatLab.longThread.fixtureShape")}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {fixture.sidebarStats.map(([label, value]) => (
                <div key={label} className="flex items-center justify-between gap-3 text-sm">
                  <span className="text-muted-foreground">{t(`issueChatLab.longThread.sidebarStats.${label === "Markdown bodies" ? "markdownBodies" : label === "Timeline events" ? "timelineEvents" : label === "Linked runs" ? "linkedRuns" : "comments"}`)}</span>
