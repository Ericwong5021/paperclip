...
const highlights = [
  "issueChatLab.highlights.runningReplies",
  "issueChatLab.highlights.historicalEvents",
  "issueChatLab.highlights.queuedMessages",
  "issueChatLab.highlights.submittingBubble",
  "issueChatLab.highlights.emptyStates",
];
...
function RotatingReasoningDemo({ intervalMs = 2200, lines }: { intervalMs?: number; lines: string[] }) {
...
export function IssueChatUxLab() {
  ...
  const reasoningLines = t("issueChatLab.reasoningLines", { returnObjects: true }) as string[];
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-(--gtc-39)">
          <div className="p-6 sm:p-7">
            <div className="inline-flex items-center gap-2 rounded-full border border-cyan-500/25 bg-cyan-500/[0.08] px-3 py-1 text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-caps) text-cyan-700 dark:text-cyan-300">
              ...
              {t("issueChatLab.chatUxLab")}
            </div>
            <h1 className="mt-4 text-3xl font-semibold tracking-tight">{t("issueChatLab.reviewSurfaceTitle")}</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">
              {t("issueChatLab.reviewSurfaceDescription")}
            </p>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2">
              ...
              <Badge variant="outline" className="rounded-full px-3 py-1 text-(length:--text-nano) uppercase tracking-(--tracking-caps)">
                {t("issueChatLab.fixtureBackedLiveRun")}
              </Badge>
            </div>

            <div className="mt-6 flex flex-wrap items-center gap-3">
              <Button variant="outline" size="sm" className="rounded-full" onClick={() => setShowComposer((value) => !value)}>
                {showComposer ? t("issueChatLab.hideComposer") : t("issueChatLab.showComposer")}
              </Button>
              <a
                href="#live-execution"
              ...
              >
                ...
                {t("issueChatLab.jumpToLiveExecution")}
              </a>
            </div>
          </div>
      ...
      <LabSection
        id="rotating-text"
        eyebrow={t("issueChatLab.animationDemo")}
        title={t("issueChatLab.rotatingReasoning")}
        description={t("issueChatLab.rotatingReasoningDescription")}
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-border/60 bg-accent/10 p-4">
            <div className="mb-2 text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
              {t("issueChatLab.defaultInterval")}
            </div>
            <RotatingReasoningDemo lines={reasoningLines} />
          </div>
          <div className="rounded-xl border border-border/60 bg-accent/10 p-4">
            <div className="mb-2 text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
              {t("issueChatLab.fastInterval")}
            </div>
            <RotatingReasoningDemo intervalMs={1000} lines={reasoningLines} />
          </div>
        </div>
      </LabSection>
      ...
      <LabSection
        id="working-tokens"
        eyebrow={t("issueChatLab.statusTokens")}
        title={t("issueChatLab.workingWorked")}
        description={t("issueChatLab.workingWorkedDescription")}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-xl border border-border/60 bg-accent/10 p-4">
            <div className="mb-3 text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
              {t("issueChatLab.activeRunShimmer")}
            </div>
            <div className="flex items-center gap-2.5 rounded-lg px-1 py-2">
              <span className="inline-flex items-center gap-2 text-sm font-medium text-foreground/80">
                <Loader2 className="h-4 w-4 shrink-0 animate-spin text-muted-foreground" />
                <span className="shimmer-text">{t("issueChatLab.working")}</span>
              </span>
              <span className="text-xs text-muted-foreground/60">{t("issueChatLab.duration12s")}</span>
            </div>
          </div>
          <div className="rounded-xl border border-border/60 bg-accent/10 p-4">
            <div className="mb-3 text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
              {t("issueChatLab.completedRunStatic")}
            </div>
            <div className="flex items-center gap-2.5 rounded-lg px-1 py-2">
              <span className="inline-flex items-center gap-2 text-sm font-medium text-foreground/80">
                ...
                {t("issueChatLab.worked")}
              </span>
              <span className="text-xs text-muted-foreground/60">{t("issueChatLab.duration1m24s")}</span>
            </div>
          </div>
        </div>
      </LabSection>
      ...
      <LabSection
        id="live-execution"
        eyebrow={t("issueChatLab.primaryPreview")}
        title={t("issueChatLab.liveExecutionThread")}
        description={t("issueChatLab.liveExecutionDescription")}
      ...
      <LabSection
        eyebrow={t("issueChatLab.submittingState")}
        title={t("issueChatLab.pendingMessageBubble")}
        description={t("issueChatLab.pendingMessageDescription")}
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <LabSection
          ...
          title={t("issueChatLab.durableCommentsFeedback")}
          description={t("issueChatLab.durableCommentsDescription")}
        ...
        <div className="space-y-6">
          <LabSection
            eyebrow={t("issueChatLab.emptyThread")}
            title={t("issueChatLab.emptyStateDisabledComposer")}
            description={t("issueChatLab.emptyStateDescription")}
          ...
          >
            <IssueChatThread
              ...
              currentUserId="user-1"
              onAdd={noop}
              composerDisabledReason={t("issueChatLab.composerDisabledReason")}
          ...
          <Card className="gap-4 border-border/70 bg-background/85 py-0">
            <CardHeader className="px-5 pt-5 pb-0">
              <div className="flex items-center gap-2 text-(length:--text-micro) font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
                ...
                {t("issueChatLab.reviewChecklist")}
              </div>
              <CardTitle className="text-lg">{t("issueChatLab.evaluatePage")}</CardTitle>
              <CardDescription>
                {t("issueChatLab.evaluatePageDescription")}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 px-5 pb-5 pt-0 text-sm text-muted-foreground">
              <div className="rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
                <div className="mb-1 flex items-center gap-2 font-medium text-foreground">
                  ...
                  {t("issueChatLab.messageHierarchy")}
                </div>
                {t("issueChatLab.messageHierarchyDescription")}
              </div>
              <div className="rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
                <div className="mb-1 flex items-center gap-2 font-medium text-foreground">
                  <Sparkles className="h-4 w-4 text-cyan-700 dark:text-cyan-300" />
                  {t("issueChatLab.streamPolish")}
                </div>
                {t("issueChatLab.streamPolishDescription")}
              </div>
            </CardContent>
          </Card>
        </div>
