...
function createRunCancelledStatusUpdateError(err: unknown): StopAndFinalizeRunError {
  const message = err instanceof Error
    ? t("taskDetail.stopUpdateFailed", { message: err.message })
    : t("taskDetail.stopUpdateFailedRetry");
...
const TREE_CONTROL_MODE_LABEL: Record<IssueTreeControlMode, string> = {
  pause: t("taskDetail.pauseSubtree"),
  resume: t("taskDetail.resumeSubtree"),
  cancel: t("taskDetail.cancelSubtree"),
  restore: t("taskDetail.restoreSubtree"),
};
const LEAF_WORK_CONTROL_MODE_LABEL: Partial<Record<IssueTreeControlMode, string>> = {
  pause: t("taskDetail.pauseWork"),
  resume: t("taskDetail.resumeWork"),
};
const TREE_CONTROL_MODE_HELP_TEXT: Record<IssueTreeControlMode, string> = {
  pause: t("taskDetail.pauseSubtreeHint"),
  resume: t("taskDetail.resumeSubtreeHint"),
  cancel: t("taskDetail.cancelSubtreeHint"),
  restore: t("taskDetail.restoreSubtreeHint"),
};
const LEAF_WORK_CONTROL_MODE_HELP_TEXT: Partial<Record<IssueTreeControlMode, string>> = {
  pause: t("taskDetail.pauseWorkHint"),
  resume: t("taskDetail.resumeWorkHint"),
};
...
function treeControlPreviewErrorCopy(error: unknown): string {
  if (error instanceof ApiError) {
    if (error.status === 403) return t("taskDetail.previewForbidden");
    if (error.status === 409) return t("taskDetail.previewStale");
    if (error.status === 422) return t("taskDetail.previewInvalid");
  }
  return error instanceof Error ? error.message : t("taskDetail.unableToLoadPreview");
}
...
function ActorIdentity({ evt, agentMap, userProfileMap }: { evt: ActivityEvent; agentMap: Map<string, Agent>; userProfileMap?: Map<string, import("../lib/company-members").CompanyUserProfile> }) {
  ...
  if (evt.actorType === "system") return <Identity name={t("taskDetail.system")} size="sm" />;
  if (evt.actorType === "user") {
    const profile = userProfileMap?.get(id);
    return <Identity name={profile?.label ?? t("taskDetail.board")} avatarUrl={profile?.image} size="sm" />;
  }
  return <Identity name={id || t("taskDetail.unknown")} size="sm" />;
}
...
function AttributionAvatar({
  ...
  const accessibleLabel = via
    ? `${label}: ${actor.name} · ${t("taskDetail.via")} ${via}`
    : `${label}: ${actor.name}`;
      ...
      <TooltipContent side="top" sideOffset={6} className="px-2 py-1.5">
        <div className="flex items-center gap-2" data-testid={`issue-${testIdLabel}-tooltip`}>
          ...
          <div className="min-w-0">
            ...
            {via ? (
              <div className="max-w-48 truncate text-(length:--text-nano) leading-3 text-background/60">{t("taskDetail.via")} {via}</div>
            ) : null}
          </div>
        </div>
      </TooltipContent>
...
function IssueAttributionByline({
  ...
  return (
    <TooltipProvider>
      <AvatarGroup className="-space-x-1.5" aria-label={t("taskDetail.taskPeople")} data-testid="issue-attribution-avatar-stack">
        {assignee ? <AttributionAvatar label={t("taskDetail.assignee")} actor={assignee} /> : null}
        {originator ? <AttributionAvatar label={t("taskDetail.originating")} actor={originator} via={originatorVia} /> : null}
      </AvatarGroup>
    </TooltipProvider>
  );
}
            ...
            <>
              ...
              {headerSeed.originKind === "routine_execution" && headerSeed.originId ? (
                <Badge variant="outline"
                  ...
                  title={t("issueDetailResidual.routineExecutionFrom", { id: headerSeed.originId })}
                >
                  <Repeat className="h-3 w-3" />
                  {t("issueDetailResidual.routine")}
                </Badge>
              ) : null}
              ...
              {headerSeed.projectId ? (
                ...
                <span className="inline-flex items-center gap-1 text-xs text-muted-foreground opacity-50 px-1 -mx-1 py-0.5">
                  <ProjectTile size="xs" />
                  {t("issueDetailResidual.noProject")}
                </span>
              )}
            </>
          ) : (
  ...
  return (
    <div className="flex items-center w-full">
      <Button
        ...
        aria-label={t("taskDetail.backToInbox")}
      >
      ...
      <div className="ml-auto flex items-center gap-0.5">
        ...
        <Popover open={menuOpen} onOpenChange={setMenuOpen}>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label={t("taskDetail.moreActions")}>
          ...
          <PopoverContent className="w-44 p-1" align="end">
            ...
            {issueIdProp && (
              ...
              >
                ...
                {t("issueDetailResidual.hideTask")}
              </button>
            )}
          </PopoverContent>
        </Popover>
...
const IssueDetailChatTab = memo(function IssueDetailChatTab({
  ...
  const loadOlderButton = hasOlderComments ? (
    <div className="flex justify-center">
      ...
      >
        {commentsLoadingOlder ? t("interactions.loadingEarlierComments") : t("interactions.loadEarlierComments")}
      </Button>
    </div>
  ) : null;
  ...
  return (
    <div className={taskChatRedesignEnabled ? "flex min-h-0 flex-1 flex-col" : "space-y-3"}>
      ...
      <ThreadComponent
        ...
        stopRunLabel={t("taskDetail.pauseWork")}
        stoppingRunLabel={t("taskDetail.pausing")}
    ...
    <>
      {shouldShowCostSummary && (
        <div className="mb-3 px-3 py-2 rounded-lg border border-border">
          <div className="text-sm font-medium text-muted-foreground mb-1">{t("taskDetail.costSummary")}</div>
          {!issueCostSummary.hasCost && !issueCostSummary.hasTokens && !hasIssueTreeCost ? (
            <div className="text-xs text-muted-foreground">{t("taskDetail.noCostData")}</div>
          ) : (
            <div className="space-y-1 text-xs text-muted-foreground tabular-nums">
              <div className="flex flex-wrap gap-3">
                <span className="font-medium text-foreground">{t("taskDetail.thisTask")}</span>
                ...
                <span>
                  {t("issueDetailResidual.tokens")} {formatTokens(issueCostSummary.totalTokens)}
                    {issueCostSummary.cached > 0
                      ? ` (${t("issueDetailResidual.input")} ${formatTokens(issueCostSummary.input)}, ${t("issueDetailResidual.output")} ${formatTokens(issueCostSummary.output)}, ${t("issueDetailResidual.cached")} ${formatTokens(issueCostSummary.cached)})`
                      : ` (${t("issueDetailResidual.input")} ${formatTokens(issueCostSummary.input)}, ${t("issueDetailResidual.output")} ${formatTokens(issueCostSummary.output)})`}
                  </span>
                ) : null}
                {issueCostSummary.hasRuntime ? (
                  <span>
                    {t("issueDetailResidual.runtime")} {formatDurationMs(issueCostSummary.runtimeMs)}
                    {` (${issueCostSummary.runCount} ${t(issueCostSummary.runCount === 1 ? "issueDetailResidual.run" : "issueDetailResidual.runs")})`}
                  </span>
                ) : null}
                {!issueCostSummary.hasCost && !issueCostSummary.hasTokens && !issueCostSummary.hasRuntime ? (
                  <span>{t("taskDetail.noDirectCost")}</span>
                ) : null}
              </div>
              {hasIssueTreeCost && issueTreeCostSummary ? (
                <div className="flex flex-wrap gap-3">
                  <span className="font-medium text-foreground">
                    {t("issueDetailResidual.includingSubTasks")} {(issueTreeCostSummary.costCents / 100).toLocaleString(undefined, {
                      style: "currency",
                      currency: "USD",
                  ...
                  <span>
                    {t("issueDetailResidual.tokens")} {formatTokens(issueTreeCostTokens)}
                    {issueTreeCostSummary.cachedInputTokens > 0
                      ? ` (${t("issueDetailResidual.input")} ${formatTokens(issueTreeCostSummary.inputTokens)}, ${t("issueDetailResidual.output")} ${formatTokens(issueTreeCostSummary.outputTokens)}, ${t("issueDetailResidual.cached")} ${formatTokens(issueTreeCostSummary.cachedInputTokens)})`
                      : ` (${t("issueDetailResidual.input")} ${formatTokens(issueTreeCostSummary.inputTokens)}, ${t("issueDetailResidual.output")} ${formatTokens(issueTreeCostSummary.outputTokens)})`}
                  </span>
                  {issueTreeCostSummary.runCount > 0 ? (
                    <span>
                      {t("issueDetailResidual.runtime")} {formatDurationMs(issueTreeCostSummary.runtimeMs)}
                      {` (${issueTreeCostSummary.runCount} ${t(issueTreeCostSummary.runCount === 1 ? "issueDetailResidual.run" : "issueDetailResidual.runs")})`}
                    </span>
                  ) : null}
                  <span>{t("issueDetailResidual.taskCount", { count: issueTreeCostSummary.issueCount, suffix: issueTreeCostSummary.issueCount === 1 ? "" : "s" })}</span>
                </div>
              ) : null}
            </div>
          )}
...
export function IssueDetail() {
  ...
  const sourceBreadcrumb = useMemo(
    () => readIssueDetailBreadcrumb(issueId, location.state, location.search) ?? { label: t("issueDetailResidual.tasks"), href: "/issues" },
    [issueId, location.state, location.search],
  );
  ...
  const commentReassignOptions = useMemo(() => {
    ...
    if (currentUserId) {
      options.push({ id: `user:${currentUserId}`, label: t("issueDetailResidual.me") });
    }
    return options;
  ...
  const breadcrumbTitle = issue?.title ?? issueId ?? t("issueDetailResidual.task");
  ...
  const updateIssue = useMutation({
    ...
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("issueDetailResidual.taskUpdateFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToSaveTaskChanges"),
        tone: "error",
      });
    },
    onSettled: () => {
  ...
  const resolveRecoveryAction = useMutation({
    ...
    onError: (err) => {
      pushToast({
        title: t("issueDetailResidual.recoveryResolutionFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToResolveRecoveryAction"),
        tone: "error",
      });
    },
    onSettled: () => {
  ...
  const executeTreeControl = useMutation({
    mutationFn: async () => {
      if (treeControlMode === "resume") {
        ...
        if (!pauseHoldId) {
          throw new Error(t("issueDetailResidual.noActiveSubtreePauseHold"));
        }
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        title: result.kind === "release"
          ? treeControlScope === "leaf" ? t("issueDetailResidual.workResumed") : t("issueDetailResidual.subtreeResumed")
          : result.hold.mode === "pause"
            ? treeControlScope === "leaf" ? t("issueDetailResidual.workPaused") : t("issueDetailResidual.subtreePaused")
            : `${modeLabel} applied`,
        body: result.kind === "release"
          ? (result.hold.releaseReason?.trim() || (treeControlScope === "leaf" ? t("issueDetailResidual.activeTaskPauseReleased") : t("issueDetailResidual.activeSubtreePauseReleased")))
          : result.hold.mode === "pause"
            ? treeControlScope === "leaf"
              ? `${t("issueDetailResidual.workPaused")} ${t("issueDetailResidual.cancelledRuns", { count: cancelCount, suffix: cancelCount === 1 ? "" : "s" })}`
              : `${t("issueDetailResidual.subtreePaused")} ${t("issueDetailResidual.cancelledRuns", { count: cancelCount, suffix: cancelCount === 1 ? "" : "s" })}`
            : result.hold.reason?.trim()
              ? result.hold.reason
              : t("issueDetailResidual.subtreeControlApplied"),
      });
    ...
    onError: (err) => {
      pushToast({
        title: t("issueDetailResidual.unableToApplySubtreeControl"),
        body: err instanceof Error ? err.message : t("taskDetail.retry"),
        tone: "error",
      });
    },
  });
  const pauseIssueWorkRun = useMutation({
    mutationFn: async ({ runId, scope }: { runId: string; scope: "leaf" | "subtree" }) => {
      const created = await issuesApi.createTreeHold(issueId!, {
        mode: "pause",
        reason: t("issueDetailResidual.pausedFromActiveRunControls"),
    ...
    onSuccess: async (result) => {
      const cancelCount = result.preview?.totals.activeRuns ?? 0;
      pushToast({
        title: t("issueDetailResidual.workPaused"),
        body: cancelCount > 0
          ? `${t("issueDetailResidual.workPaused")} ${t("issueDetailResidual.cancelledRuns", { count: cancelCount, suffix: cancelCount === 1 ? "" : "s" })}`
          : t("issueDetailResidual.workHeldUntilResume"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.issues.detail(issueId!) }),
    ...
    onError: (err) => {
      pushToast({
        title: t("issueDetailResidual.unableToPauseWork"),
        body: err instanceof Error ? err.message : t("taskDetail.retry"),
        tone: "error",
      });
    },
  ...
  const stopAndFinalizeRun = useMutation({
    ...
    onSuccess: ({ comment: _comment, ...nextIssue }, { status }) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: status === "done" ? t("issueDetailResidual.runStoppedAndTaskDone") : t("issueDetailResidual.runStoppedAndTaskCancelled"),
        tone: "success",
      });
    },
    onError: (err, { status }) => {
      ...
      pushToast({
        title: runWasStopped
          ? t("issueDetailResidual.runStoppedTaskUpdateFailed")
          : status === "done" ? t("issueDetailResidual.stopAndDoneFailed") : t("issueDetailResidual.stopAndCancelFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToStopRunAndUpdateTask"),
        tone: "error",
      });
    },
    onSettled: (_data, err) => {
  ...
  const updateChildIssue = useMutation({
    ...
    onError: (err) => {
      pushToast({
        title: t("issueDetailResidual.taskUpdateFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToSaveSubTaskChanges"),
        tone: "error",
      });
    },
  });
  ...
  const checkIssueMonitorNow = useMutation({
    ...
    onSuccess: () => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("issueDetailResidual.monitorCheckQueued"),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetailResidual.monitorCheckFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToTriggerMonitor"),
        tone: "error",
      });
    },
  });
  ...
  const approvalDecision = useMutation({
    ...
    onSuccess: (_approval, variables) => {
      ...
      pushToast({
        title: variables.action === "approve" ? t("issueDetailResidual.approvalApproved") : t("issueDetailResidual.approvalRejected"),
        tone: "success",
      });
    },
    onError: (err, variables) => {
      pushToast({
        title: variables.action === "approve" ? t("issueDetailResidual.approvalFailed") : t("issueDetailResidual.rejectionFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToUpdateApproval"),
        tone: "error",
      });
    },
    onSettled: () => {
  ...
  const addComment = useMutation({
    ...
    onSuccess: async (comment, _variables, context) => {
      ...
      if (context?.optimisticCommentId && cancelledQueuedOptimisticCommentIdsRef.current.has(context.optimisticCommentId)) {
        ...
        try {
          ...
          pushToast({
            title: t("issueDetailResidual.cancelFailed"),
            body: err instanceof Error ? err.message : t("issueDetailResidual.unableToCancelQueuedComment"),
            tone: "error",
          });
        }
      }
    ...
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("issueDetailResidual.commentFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToPostComment"),
        tone: "error",
      });
    },
    ...
    onSuccess: (interaction) => {
      ...
      pushToast({
        title: interaction.kind === "request_confirmation"
          ? t("issueDetailResidual.requestConfirmed")
          : interaction.kind === "request_checkbox_confirmation"
          ? t("issueDetailResidual.selectionConfirmed")
          : skippedCount > 0
          ? t("issueDetailResidual.acceptedDrafts", { createdCount, createdSuffix: createdCount === 1 ? "" : "s", skippedCount })
          : t("issueDetailResidual.suggestedTasksAccepted"),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetailResidual.acceptFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToAcceptSuggestedTasks"),
        tone: "error",
      });
    },
  });
  const rejectInteraction = useMutation({
    ...
    onSuccess: (interaction) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: interaction.kind === "request_confirmation" ? t("issueDetailResidual.requestDeclined") : t("issueDetailResidual.suggestionRejected"),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetailResidual.rejectFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToRejectSuggestedTasks"),
        tone: "error",
      });
    },
  });
  const answerInteraction = useMutation({
    ...
    onSuccess: (interaction) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("issueDetailResidual.answersSubmitted"),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetailResidual.submitFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToSubmitAnswers"),
        tone: "error",
      });
    },
  });
  ...
  const submitInteractionVerdicts = useMutation({
    ...
    onSuccess: (interaction, variables) => {
      ...
      pushToast({
        title: complete
          ? t("issueDetailResidual.allVerdictsApplied")
          : t("issueDetailResidual.appliedDecisions", { count: applied, suffix: applied === 1 ? "" : "s" }),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetailResidual.applyFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToApplyVerdicts"),
        tone: "error",
      });
    },
  });
  ...
  const cancelInteraction = useMutation({
    ...
    onSuccess: (interaction) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("issueDetailResidual.questionCancelled"),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetailResidual.cancelFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToCancelQuestion"),
        tone: "error",
      });
    },
  });
  ...
  const interruptQueuedComment = useMutation({
    ...
    onSuccess: () => {
      ...
      invalidateIssueRunState();
      pushToast({
        title: t("issueDetailResidual.interruptRequested"),
        body: t("issueDetailResidual.activeRunStopping"),
        tone: "success",
      });
    },
    onError: (err, _runId, context) => {
      ...
      pushToast({
        title: t("issueDetailResidual.interruptFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToInterruptActiveRun"),
        tone: "error",
      });
    },
  });
  ...
  const cancelQueuedComment = useMutation({
    ...
    onSuccess: (comment) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("issueDetailResidual.queuedCommentCancelled"),
        body: t("issueDetailResidual.queuedMessageRestored"),
        tone: "success",
      });
    },
    onError: (err) => {
  ...
  const deleteComment = useMutation({
    ...
    onSuccess: (comment) => {
      ...
      pushToast({
        title: t("issueDetailResidual.commentDeleted"),
        body: t("issueDetailResidual.deletedCommentMarker"),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetailResidual.deleteFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToDeleteComment"),
        tone: "error",
      });
    },
  });
        ...
        title:
          variables.sharingPreferenceAtSubmit === "prompt"
            ? variables.allowSharing
              ? t("issueDetailResidual.feedbackSavedFutureSharing")
              : t("issueDetailResidual.feedbackSavedLocal")
            : variables.allowSharing
              ? t("issueDetailResidual.feedbackSavedSharingEnabled")
              : t("issueDetailResidual.feedbackSaved"),
        tone: "success",
      });
    },
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("issueDetailResidual.failedToSaveFeedback"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unknownError"),
        tone: "error",
      });
    },
  });
  ...
  const uploadAttachment = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) throw new Error(t("issueDetailResidual.noCompanySelected"));
    ...
    onError: (err) => {
      setAttachmentError(err instanceof Error ? err.message : t("issueDetailResidual.uploadFailed"));
    },
  });
  ...
  const importMarkdownDocument = useMutation({
    ...
    onError: (err) => {
      setAttachmentError(err instanceof Error ? err.message : t("issueDetailResidual.documentImportFailed"));
    },
  });
  ...
  const deleteAttachment = useMutation({
    ...
    onError: (err) => {
      setAttachmentError(err instanceof Error ? err.message : t("issueDetailResidual.deleteFailed"));
    },
  });
  ...
  const archiveFromInbox = useMutation({
    ...
    onSuccess: (_data, id) => {
      ...
      pushToast({ title: t("issueDetailResidual.taskArchivedFromInbox"), tone: "success" });
    },
    onError: (err, id, context) => {
      ...
      pushToast({
        title: t("issueDetailResidual.archiveFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToArchiveTask"),
        tone: "error",
      });
    },
  ...
  const copyIssueToClipboard = async () => {
    ...
    try {
      ...
      setCopied(true);
      pushToast({ title: t("issueDetailResidual.copiedToClipboard"), tone: "success" });
      setTimeout(() => setCopied(false), 2000);
    } catch (error) {
      pushToast({
        title: t("issueDetailResidual.copyFailed"),
        body: error instanceof Error ? error.message : t("issueDetailResidual.unableToCopyTaskMarkdown"),
        tone: "error",
      });
    }
  };
  ...
  const runFinalizationActions = useMemo<readonly IssueChatRunFinalizationAction[]>(() => [
    {
      id: "cancel",
      label: t("issueDetailResidual.stopAndCancel"),
      pendingLabel: t("issueDetailResidual.stoppingAndCancelling"),
      isPending:
    ...
    {
      id: "done",
      label: t("issueDetailResidual.stopAndDone"),
      pendingLabel: t("issueDetailResidual.stoppingAndMarkingDone"),
      isPending:
        stopAndFinalizeRun.isPending &&
  ...
  const reissueIsolatedRecoveryAction = useMutation({
    mutationFn: async (
      ...
      if (!issue) throw new Error(t("issueDetailResidual.taskUpdateFailed"));
      const sourceLabel = issue.identifier ?? t("issueDetailResidual.stalledTask");
      const descriptionLines = [
        t("issueDetailResidual.reissuedFrom", { sourceLabel }),
        "",
        `- ${t("issueDetailResidual.baseRefLiveBranch", { baseRef: request.baseRef })}`,
        ...(request.expectedBranch ? [`- ${t("issueDetailResidual.recordedBranch", { branch: request.expectedBranch })}`] : []),
        "",
        "---",
        "",
        issue.description ?? "",
    ...
    onSuccess: (created) => {
      invalidateIssueCollections();
      pushToast({
        title: t("issueDetailResidual.isolatedReissueCreated"),
        body: created.identifier
          ? t("issueDetailResidual.isolatedWorkspaceRun", { identifier: created.identifier })
          : t("issueDetailResidual.freshIsolatedReissueCreated"),
        tone: "success",
      });
    ...
    onError: (err) => {
      pushToast({
        title: t("issueDetailResidual.reissueFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToCreateIsolatedReissue"),
        tone: "error",
      });
    },
  });
  ...
  const reconcileRecoveryAction = useMutation({
    ...
    onSuccess: (_result, variables) => {
      ...
      pushToast(
        variables.mode === "quarantine_restore"
          ? {
              title: t("issueDetailResidual.workspaceRepaired"),
              body: t("issueDetailResidual.workspaceRepairedBody"),
              tone: "success",
            }
          : {
              title: t("issueDetailResidual.workspaceBranchReconciled"),
              body: t("issueDetailResidual.workspaceBranchReconciledBody"),
              tone: "success",
            },
      );
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetailResidual.reconcileFailed"),
        body: err instanceof Error ? err.message : t("issueDetailResidual.unableToReconcileWorkspaceBranch"),
        tone: "error",
      });
    },
  });
  ...
  const handleReconcileForwardRecoveryAction = useCallback(() => {
    if (!reconcileExecutionWorkspaceId) {
      pushToast({
        title: t("issueDetailResidual.reconcileFailed"),
        body: t("issueDetailResidual.noExecutionWorkspaceToReconcile"),
        tone: "error",
      });
      return;
    }
  ...
  const handleQuarantineRestoreRecoveryAction = useCallback(() => {
    if (!reconcileExecutionWorkspaceId) {
      pushToast({
        title: t("issueDetailResidual.repairFailed"),
        body: t("issueDetailResidual.noExecutionWorkspaceToRepair"),
        tone: "error",
      });
      return;
    }
  ...
  const childPauseBadgeById = useMemo(() => {
    ...
    for (const child of childIssues) {
      ...
      badges.set(child.id, t("issueDetailResidual.workPaused"));
    }
    return badges;
  }, [childIssues, heldIssueIds]);
  ...
  const treeControlPrimaryButtonLabel =
    treeControlMode === "pause"
      ? treeControlScope === "leaf"
        ? t("taskDetail.pauseWork")
        : t("issueDetailResidual.pauseAndStopWork")
      : treeControlMode === "cancel"
        ? t("issueDetailResidual.cancelTasks", { count: previewAffectedIssueCount })
      : treeControlMode === "restore"
          ? t("issueDetailResidual.restoreTasks", { count: previewAffectedIssueCount })
          : treeControlScope === "leaf"
            ? t("taskDetail.resumeWork")
            : t("taskDetail.resumeSubtree");
  ...
  const pausedComposerHint = activePauseHold
    ? (
      issue.assigneeAgentId
        ? t("issueDetailResidual.commentWakesAssignee", { name: agentMap.get(issue.assigneeAgentId)?.name ?? t("taskDetail.assignee") })
        : t("issueDetailResidual.assignAgentForTriage")
    )
    : null;
  ...
  const attachmentUploadButton = (
    <>
      ...
      >
        ...
        {uploadAttachment.isPending || importMarkdownDocument.isPending ? t("issueDetailResidual.uploading") : (
          <>
            <span className="hidden sm:inline">{t("taskDetail.uploadAttachment")}</span>
            <span className="sm:hidden">{t("taskDetail.upload")}</span>
          </>
        )}
      </Button>
    </>
        ...
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          ...
          {issue.originKind === "routine_execution" && issue.originId && (
            <Link
              ...
              title={t("issueDetailResidual.routineExecutionFrom", { id: issue.originId })}
            >
              <Repeat className="h-3 w-3" />
              {t("issueDetailResidual.routine")}
            </Link>
          ...
          {issue.originKind === "issue_productivity_review" ? (
            <Badge variant="outline"
              ...
              title={t("taskDetail.productivityReview")}
            >
              <Eye className="h-3 w-3" />
              {t("issueDetailResidual.productivityReview")}
            </Badge>
          ) : null}
          ...
          {issue.originKind === "task_watchdog" ? (
            <Badge variant="outline"
              ...
              title={t("taskDetail.watchdogTask")}
            >
              <ScanEye className="h-3 w-3" />
              {t("issueDetailResidual.watchdog")}
            </Badge>
          ) : null}
          ...
          {!taskChatShellEnabled && (issue.workMode === "ask" || issue.workMode === "planning") ? (() => {
            ...
            return (
              <Badge variant="outline"
                ...
                title={t("issueDetailResidual.workModeTitle", { mode: workModeMeta.label.toLowerCase() })}
              >
          ...
          {hasAssignedBacklogBlocker(issue.blockedBy) ? (
            <Badge variant="outline"
              ...
              className="border-amber-500/60 bg-amber-500/15 text-(length:--text-nano) text-amber-700 dark:text-amber-300"
                title={t("taskDetail.blockedByParkedWork")}
            >
              <Flag className="h-3 w-3" />
              {t("issueDetailResidual.blockedByParkedWork")}
            </Badge>
          ) : null}
          ...
          {!(isMobile && isFromInbox) && (
            <div className="ml-auto flex items-center gap-0.5 md:hidden shrink-0">
              <Button
                ...
                title={t("taskDetail.copyTaskMarkdown")}
              >
          ...
          <div className="hidden md:flex items-center md:ml-auto shrink-0">
            ...
            {fileViewerEnabled ? (
              ...
              aria-label={t("taskDetail.openFileInTask")}
              >
            ...
            <Button
              ...
              title={t("taskDetail.showProperties")}
            >
              <SlidersHorizontal className="h-4 w-4" />
            </Button>
            ...
            <Popover open={moreOpen} onOpenChange={setMoreOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  size="icon-xs"
                  className="shrink-0"
                  aria-label={t("taskDetail.taskActions")}
                  title={t("taskDetail.taskActions")}
                  onKeyDown={(event) => {
            ...
            <PopoverContent className="w-52 p-1" align="end">
              {canPauseLeafWork ? (
                ...
                >
                  ...
                  {t("issueDetailResidual.pauseWorkMenu")}
                </button>
              ) : null}
              {canResumeLeafWork ? (
                <button
                ...
                >
                  ...
                  {t("issueDetailResidual.resumeWorkMenu")}
                </button>
              ) : null}
              {canShowSubtreeControls ? (
                <>
                  ...
                  >
                    <PauseCircle className="h-3 w-3" />
                    {t("issueDetailResidual.pauseSubtreeMenu")}
                  </button>
                  {canResumeSubtree ? (
                    <button
                      className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50"
                    ...
                    >
                      <PlayCircle className="h-3 w-3" />
                      {t("issueDetailResidual.resumeSubtreeMenu")}
                    </button>
                  ) : null}
                  <button
                    className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-destructive"
                  ...
                  >
                    ...
                    {t("issueDetailResidual.cancelSubtreeMenu")}
                  </button>
                  {canRestoreSubtree ? (
                    <button
                      className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50"
                    ...
                    >
                      <Repeat className="h-3 w-3" />
                      {t("issueDetailResidual.restoreSubtreeMenu")}
                    </button>
                  ) : null}
                </>
              ) : null}
    ...
    >
      ...
      {issue.hiddenAt && (
        <div className={cn("flex items-center gap-2 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive", shellSectionClass)}>
          ...
          {t("issueDetailResidual.hiddenTask")}
        </div>
      )}
      {activePauseHold && (
        <div className={cn("rounded-md border border-amber-500/35 bg-amber-500/10 p-3 text-sm text-amber-800 dark:text-amber-200", shellSectionClass)}>
          {activePauseHold.isRoot ? (
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-medium">
                  {childIssues.length === 0 ? t("issueDetailResidual.pausedByBoard") : t("issueDetailResidual.subtreePauseActive")}
                </span>
                <span className="text-xs text-amber-900/80 dark:text-amber-100/80">
                  {childIssues.length === 0
                    ? t("issueDetailResidual.taskExecutionHeld")
                    : t("issueDetailResidual.subtreeExecutionHeld")}
                </span>
              </div>
              <div className="text-xs text-amber-900/80 dark:text-amber-100/80">
                {childIssues.length === 0
                  ? t("issueDetailResidual.taskHeld")
                  : t("issueDetailResidual.descendantsHeld", { count: heldDescendantCount, suffix: heldDescendantCount === 1 ? "" : "s" })}
                {activeRootPauseHold?.createdAt ? ` · ${t("issueDetailResidual.started", { time: relativeTime(activeRootPauseHold.createdAt) })}` : ""}
              </div>
              {canShowSubtreeControls || canResumeLeafWork ? (
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {childIssues.length === 0 ? t("taskDetail.resumeWork") : t("taskDetail.resumeSubtree")}
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"
                  ...
                  >
                    {t("issueDetailResidual.viewAffected", { count: childIssues.length === 0 ? 1 : heldDescendantCount })}
                  </Button>
                  {canShowSubtreeControls ? (
                    <Button
                      variant="ghost"
            ...
            <div className="text-xs">
              {t("issueDetailResidual.taskPausedByAncestor")} {" "}
              ...
              {t("issueDetailResidual.resumeFromRoot")}
            </div>
          )}
        </div>
      )}
      ...
      {taskChatShellEnabled ? null : showRichSubIssuesSection ? (
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-2">
              <h3 className="text-sm font-medium text-muted-foreground">{t("taskDetail.subTasks")}</h3>
          </div>
          <IssuesList
            issues={childIssues}
            ...
            createIssueLabel={t("issueDetailResidual.newSubTask")}
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
          <Button variant="outline" size="sm" onClick={openNewSubIssue} className="shrink-0 shadow-none">
            ...
            {t("issueDetailResidual.newSubTask")}
          </Button>
        </div>
      )}
      ...
      {!taskChatShellEnabled && fileViewerEnabled && issue.workProducts && issue.workProducts.length > 0 && (() => {
        ...
        return (
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-2">
              <h3 className="text-sm font-medium text-muted-foreground">{t("taskDetail.artifacts")}</h3>
            </div>
      ...
      >
        ...
        <TabsList variant="line" className={cn("w-full justify-start gap-1", shellSectionClass)}>
          <TabsTrigger value="chat" className="gap-1.5">
            ...
            {t("issueDetailResidual.chat")}
          </TabsTrigger>
          <TabsTrigger value="activity" className="gap-1.5">
            <ActivityIcon className="h-3.5 w-3.5" />
            {t("issueDetailResidual.activity")}
          </TabsTrigger>
          <TabsTrigger value="related-work" className="gap-1.5">
            <ListTree className="h-3.5 w-3.5" />
            {t("issueDetailResidual.relatedWork")}
          </TabsTrigger>
      ...
      <Dialog open={treeControlOpen} onOpenChange={setTreeControlOpen}>
        <DialogContent className="flex max-h-(--sz-calc-18) flex-col gap-0 overflow-hidden p-0 sm:max-w-(--sz-560px)">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
            {treeControlMode === "cancel" ? (
              <div className="rounded-md border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive">
                {t("issueDetailResidual.cancellingSubtree")}
              </div>
            ) : null}

            <div className="space-y-1.5">
              <label className="text-xs text-muted-foreground">
                {t("issueDetailResidual.reasonOptional")}
              </label>
              <Textarea
                value={treeControlReason}
                onChange={(event) => setTreeControlReason(event.target.value)}
                placeholder={t("taskDetail.explainTreeControl")}
            ...
            {(treeControlMode === "resume" || treeControlMode === "restore") ? (
              <div className="space-y-2">
                <label className="flex items-start gap-2 text-sm">
                  ...
                  />
                  <span>
                    <span className="block font-medium">{t("issueDetailResidual.wakeAffectedAgents", { count: previewAffectedAgentCount })}</span>
                    <span className="text-xs text-muted-foreground">
                      {previewAffectedAgentCount === 0
                        ? t("issueDetailResidual.noEligibleAgents")
                        : t("issueDetailResidual.wakeAgentsAfterOperation")}
                    </span>
                  </span>
                </label>
            ...
            {treeControlMode === "cancel" ? (
              <label className="flex items-start gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-2 text-sm">
                ...
                />
                <span>{t("issueDetailResidual.understandCancellation", { count: previewAffectedIssueCount })}</span>
              </label>
            ) : null}
            ...
            <div className="space-y-2">
              {treeControlPreviewLoading ? (
                ...
                <div className="space-y-2">
                  ...
                  >
                    {t("issueDetailResidual.retryPreview")}
                  </Button>
                </div>
              ) : treeControlPreview ? (
                <div className="space-y-2">
                        ...
                        <div key={candidate.id} style={candidate.depth > 0 ? { paddingLeft: `${Math.min(candidate.depth, 6) * 14}px` } : undefined}>
                          ...
                          >
                            ...
                            {candidate.skipped && candidate.skipReason === "terminal_status" ? (
                              <span className="shrink-0 text-xs text-muted-foreground">{t("taskDetail.complete")}</span>
                            ) : null}
                          </Link>
                        </div>
                      ))}
                ...
                <p className="text-xs text-muted-foreground">{t("taskDetail.previewUnavailable")}</p>
              )}
            </div>
          </div>
          <DialogFooter className="border-t border-border/60 bg-background px-6 py-4">
            ...
            >
              {executeTreeControl.isPending ? t("issueDetailResidual.applying") : treeControlPrimaryButtonLabel}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Sheet open={mobilePropsOpen} onOpenChange={setMobilePropsOpen}>
        <SheetContent side="bottom" className="max-h-(--sz-85dvh) pb-(--sz-safe-bottom)">
          <SheetHeader>
            <SheetTitle className="text-sm">{t("taskDetail.properties")}</SheetTitle>
          </SheetHeader>
