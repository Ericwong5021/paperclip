...
export function JoinRequestQueue() {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("joinRequests.approvedToast"), tone: "success" });
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: queryKeys.access.joinRequests(selectedCompanyId!, `${status}:${requestType}`) });
      pushToast({ title: t("joinRequests.rejectedToast"), tone: "success" });
    },
  });
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("joinRequests.queueTitle")}</h1>
        </div>
      ...
      <Card className="flex-row flex-wrap gap-3 p-4">
        <label className="space-y-2 text-sm">
          ...
          >
            <option value="pending_approval">{t("joinRequests.statuses.pending_approval")}</option>
            <option value="approved">{t("joinRequests.statuses.approved")}</option>
            <option value="rejected">{t("joinRequests.statuses.rejected")}</option>
          </select>
        </label>
        <label className="space-y-2 text-sm">
          ...
          >
            <option value="all">{t("joinRequests.types.all")}</option>
            <option value="human">{t("joinRequests.types.human")}</option>
            <option value="agent">{t("joinRequests.types.agent")}</option>
          </select>
        </label>
      </Card>
            ...
            <Card key={request.id} className="block p-4">
                ...
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge variant={request.status === "pending_approval" ? "secondary" : request.status === "approved" ? "outline" : "destructive"}>
                      {t(`joinRequests.statuses.${request.status}`, { defaultValue: request.status.replace("_", " ") })}
                    </Badge>
                    <Badge variant="outline">{t(`joinRequests.types.${request.requestType}`, { defaultValue: request.requestType })}</Badge>
              ...
              <div className="mt-4 grid gap-3 text-sm text-muted-foreground md:grid-cols-2">
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                  ...
                  <div className="mt-2">
                    {request.invite
                      ? t("joinRequests.inviteSummary", {
                          type: t(`joinRequests.types.${request.invite.allowedJoinTypes}`, { defaultValue: request.invite.allowedJoinTypes }),
                          role: request.invite.humanRole
                            ? t("joinRequests.defaultRole", {
                                role: t(`joinRequests.roles.${request.invite.humanRole}`, { defaultValue: request.invite.humanRole }),
                              })
                            : "",
                        })
                ...
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                  ...
                  <div className="mt-2">{t("joinRequests.submitted", { time: new Date(request.createdAt).toLocaleString() })}</div>
