...
function stageNavigationText(value: string) {
  ...
  const path = `workflow.pipelineSettings.${key[value] ?? value}`;
  const translated = t(path);
  return translated === path ? value : translated;
}

function stageKindText(kind: EditableStageKind, field: "label" | "description") {
  const key = kind === "working" ? "working" : kind;
  return t(`workflow.pipelineSettings.${key}${field === "description" ? "Description" : ""}`);
}

function workspaceOptionText(value: string) {
  const key = value === "shared_workspace"
    ? "projectDefault"
    : value === "isolated_workspace"
      ? "newIsolatedWorkspace"
      : "reuseExistingWorkspace";
  return t(`workflow.pipelineSettings.${key}`);
}

function automationVariableText(key: string, field: "label" | "description") {
  const names: Record<string, string> = {
    ...
    case_id: "itemId",
    case_key: "itemKey",
    case_title: "itemTitleAlias",
    case_version: "itemVersion",
  };
  const suffix = field === "description" ? "Description" : "";
  const path = `workflow.pipelineSettings.${names[key] ?? key}${suffix}`;
  const translated = t(path);
...
function fieldOriginLabel(depth: number, pipelineName: string) {
  if (depth === 0) return t("workflow.pipelineSettings.thisItem");
...
function automationVariablePreviewTitle(variable: AutomationVariableOption) {
  ...
  const size = typeof variable.example === "string" ? `${variable.example.length} ${t("workflow.pipelineSettings.chars")}` : `${preview.length} ${t("workflow.pipelineSettings.chars")}`;
  const lines = [
    variable.description,
    `${t("workflow.pipelineSettings.example")}: ${automationVariableKind(variable.example)}, ${size}`,
  ];
  if (variable.exampleSource) lines.push(`${t("workflow.pipelineSettings.from")} ${variable.exampleSource}`);
...
function buildAutomationVariableGroups(input: {
  ...
  const pipelineVariables: AutomationVariableOption[] = [
    {
      key: "pipeline_id",
      label: t("workflow.pipelineSettings.pipelineId"),
      description: t("workflow.pipelineSettings.pipelineIdDescription"),
      example: input.pipeline.id,
      exampleSource: null,
    },
    {
      key: "pipeline_key",
      label: t("workflow.pipelineSettings.pipelineKey"),
      description: t("workflow.pipelineSettings.pipelineKeyDescription"),
      example: input.pipeline.key,
      exampleSource: null,
    },
    {
      key: "pipeline_name",
      label: t("workflow.pipelineSettings.pipelineName"),
      description: t("workflow.pipelineSettings.pipelineNameDescription"),
      example: input.pipeline.name,
      exampleSource: null,
    },
    {
      key: "stage_id",
      label: t("workflow.pipelineSettings.stageId"),
      description: t("workflow.pipelineSettings.stageIdDescription"),
      example: input.stage.id,
      exampleSource: null,
    },
    {
      key: "stage_key",
      label: t("workflow.pipelineSettings.stageKey"),
      description: t("workflow.pipelineSettings.stageKeyDescription"),
      example: input.stage.key,
      exampleSource: null,
    },
    {
      key: "stage_name",
      label: t("workflow.pipelineSettings.stageName"),
      description: t("workflow.pipelineSettings.stageNameDescription"),
  ...
  const itemVariables: AutomationVariableOption[] = [
    {
      key: "title",
      label: t("workflow.pipelineSettings.itemTitle"),
      description: t("workflow.pipelineSettings.itemTitleDescription"),
      example: sampleCase?.title ?? "",
      exampleSource,
    },
    {
      key: "body",
      label: t("workflow.pipelineSettings.itemBody"),
      description: t("workflow.pipelineSettings.itemBodyDescription"),
      example: sampleCase?.summary ?? "",
      exampleSource,
    },
    {
      key: "case_id",
      label: t("workflow.pipelineSettings.itemId"),
      description: t("workflow.pipelineSettings.itemIdDescription"),
      example: sampleCase?.id ?? "",
      exampleSource,
    },
    {
      key: "case_key",
      label: t("workflow.pipelineSettings.itemKey"),
      description: t("workflow.pipelineSettings.itemKeyDescription"),
      example: sampleCase?.caseKey ?? "",
      exampleSource,
    },
    {
      key: "case_title",
      label: t("workflow.pipelineSettings.itemTitleAlias"),
      description: t("workflow.pipelineSettings.itemTitleAliasDescription"),
      example: sampleCase?.title ?? "",
      exampleSource,
    },
    {
      key: "case_version",
      label: t("workflow.pipelineSettings.itemVersion"),
      description: t("workflow.pipelineSettings.itemVersionDescription"),
  ...
  for (const [key, value] of Object.entries(fields)) {
    ...
    fieldVariables.push({
      ...
      description: t("workflow.pipelineSettings.fieldCopied"),
      example: value,
      exampleSource,
    });
  }
  const groups: AutomationVariableGroup[] = [
    { id: "pipeline", label: t("workflow.pipelineSettings.pipelineAndStageLabel"), variables: pipelineVariables },
    { id: "item", label: t("workflow.pipelineSettings.currentItem"), variables: itemVariables },
  ];
  if (fieldVariables.length > 0) {
    groups.push({ id: "fields", label: t("workflow.pipelineSettings.itemFields"), variables: fieldVariables });
  }
  return groups;
}
...
function AutomationVariableTokenHelper({
  groups,
  onInsert,
  label = t("workflow.pipelineSettings.availableVariables"),
}: {
  ...
  return (
    <>
      <div className="md:hidden">
        <label className="sr-only" htmlFor="stage-section-picker">{t("workflow.pipelineSettings.stageSection")}</label>
        <select
        ...
        >
          {groups.map((group) => (
            <optgroup key={group.label} label={stageNavigationText(group.label)}>
              {group.items.map((item) => (
                <option key={item.id} value={item.id}>{stageNavigationText(item.label)}</option>
              ))}
            </optgroup>
          ))}
        </select>
      ...
      <nav
        aria-label={t("workflow.pipelineSettings.stageSections")}
        ...
        {groups.map((group) => (
          <div key={group.label} className="flex flex-col gap-0.5">
            <p className="px-3 py-2 text-(length:--text-micro) font-semibold uppercase tracking-(--tracking-eyebrow) text-muted-foreground/80">
              {stageNavigationText(group.label)}
            </p>
            {group.items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="truncate">{stageNavigationText(item.label)}</span>
                </button>
              );
            })}
          </div>
...
export function PipelineSettings() {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) throw new Error(t("workflow.pipelineSettings.selectCompanyCreateSecrets"));
  ...
  useEffect(() => {
    ...
    setBreadcrumbs([
      ...
      { label: t("workflow.common.settings") },
    ]);
  ...
  const saveStage = useMutation({
    mutationFn: async () => {
      ...
      if (
        ...
        throw new Error(t("workflow.pipelineSettings.chooseWorkspaceBeforeSave"));
      }
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("workflow.pipelineSettings.stageSaved"), tone: "success" });
    },
    onError: async (error) => {
      pushToast({
        title: t("workflow.pipelineSettings.saveStageFailed"),
        body: error instanceof Error ? error.message : t("workflow.pipelineSettings.saveStageError"),
        tone: "error",
      });
    },
  });
  ...
  const saveStageEnv = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("workflow.pipelineSettings.stageSecretsSaved"), tone: "success" });
    },
    onError: async (error) => {
      pushToast({
        title: t("workflow.pipelineSettings.saveSecretsFailed"),
        body: error instanceof ApiError
          ? error.message
          : error instanceof Error
            ? error.message
            : t("workflow.pipelineSettings.saveSecretsError"),
        tone: "error",
      });
    },
  });
  ...
  const addStage = useMutation({
    mutationFn: async (afterStage: PipelineStage | null) => {
      ...
      const created = await pipelinesApi.createStage(pipelineId, {
        key: nextStageKey(t("workflow.pipelineSettings.newStage"), existingKeys),
        name: t("workflow.pipelineSettings.newStage"),
        kind: "working",
    ...
    onSuccess: async (created) => {
      ...
      pushToast({ title: t("workflow.pipelineSettings.stageAdded"), tone: "success" });
    },
  });
  ...
  const deleteStage = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("workflow.pipelineSettings.stageDeleted"), tone: "success" });
    },
    onError: (error) => {
      pushToast({
        title: t("workflow.pipelineSettings.deleteStageFailed"),
        body: error instanceof Error ? error.message : t("workflow.pipelineSettings.deleteStageError"),
        tone: "error",
      });
    },
  });
  ...
  const savePipelineDetails = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("workflow.pipelineSettings.pipelineUpdated"), tone: "success" });
    },
  });
  ...
  const saveStrictTransitions = useMutation({
    ...
    onSuccess: async () => {
      await refreshPipeline();
      pushToast({ title: t("workflow.pipelineSettings.transitionRulesUpdated"), tone: "success" });
    },
    onError: (error) => {
      setStrictTransitionsEnabled(pipeline?.enforceTransitions ?? false);
      pushToast({
        title: t("workflow.pipelineSettings.updateTransitionsFailed"),
        body: error instanceof Error ? error.message : t("workflow.pipelineSettings.updateTransitionsError"),
        tone: "error",
      });
    },
  });
  ...
  const archivePipeline = useMutation({
    ...
    onSuccess: async (_result, archived) => {
      ...
      if (archived) {
        ...
        await refreshPipeline();
        pushToast({ title: t("workflow.pipelineSettings.pipelineRestored"), tone: "success" });
      }
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Hexagon} message={t("workflow.pipelineSettings.selectCompany")} />;
  }

  if (!pipelineId) {
    return <EmptyState icon={Hexagon} message={t("workflow.pipelines.pipelineNotFound")} />;
  }
  ...
  const transitionTargetsControl = !isReviewStage && !isPipelineTerminalStageKind(stageKind) ? (
    <FieldRow label={t("workflow.pipelineSettings.allowedNextSteps")}>
      <div className="space-y-2">
        {otherStages.map((stage) => {
          ...
          return (
            ...
            >
              ...
              {isCancelled ? (
                  <span className="text-xs text-muted-foreground">{t("workflow.pipelineSettings.active")}</span>
              ) : null}
            </label>
          );
        })}
    ...
    <div className="rounded-lg border border-border">
      <div className="flex items-start justify-between gap-4 border-b border-border p-4">
        <div className="space-y-1">
          <h3 className="text-sm font-semibold text-foreground">{t("workflow.pipelineSettings.breakIntoPieces")}</h3>
          <p className="max-w-md text-sm text-muted-foreground">
            {t("workflow.pipelineSettings.breakdownDescription")}
          </p>
        </div>
        <ToggleSwitch
          aria-label={t("workflow.pipelineSettings.breakIntoPieces")}
      ...
      {breakdownEnabled ? (
        <div className="divide-y divide-border px-4">
          <FieldRow label={t("workflow.pipelineSettings.createEachPieceIn")}>
            <div className="space-y-1">
              <div className="flex w-full max-w-sm items-center">
                <select
                  aria-label={t("workflow.pipelineSettings.createEachPieceIn")}
                ...
                >
                  <option value="">{t("workflow.pipelineSettings.choosePipeline")}</option>
                ...
                {breakdownTargetPipelineId ? (
                  <Link
                    ...
                    aria-label={`${t("workflow.common.open")} ${breakdownTargetPipeline?.name ?? t("workflow.pipelineSettings.selectedPipeline")} ${t("workflow.common.pipeline")}`}
                    title={`${t("workflow.common.open")} ${breakdownTargetPipeline?.name ?? t("workflow.pipelineSettings.selectedPipeline")} ${t("workflow.common.pipeline")}`}
              ...
              {!breakdownTargetPipelineId ? (
                <p className="text-xs text-muted-foreground">{t("workflow.pipelineSettings.projectWorkspace")}</p>
              ) : null}
            </div>
          </FieldRow>
          <FieldRow label={t("workflow.pipelineSettings.startingStageLabel")}>
            <div className="space-y-1">
              <select
                aria-label={t("workflow.pipelineSettings.startingStage")}
              ...
              >
                  <option value="">{t("workflow.pipelineSettings.chooseStage")}</option>
                {breakdownTargetStages.map((stage) => (
                  <option key={stage.id} value={stage.key}>{stage.name}</option>
                ))}
              </select>
              <p className="text-xs text-muted-foreground">{t("workflow.pipelineSettings.stageEveryPieceStarts")}</p>
            </div>
          </FieldRow>
          <FieldRow label={t("workflow.pipelineSettings.callEachPiece")}>
            <div className="space-y-1">
              <Input
              aria-label={t("workflow.pipelineSettings.callEachPiece")}
                value={breakdownPieceNoun}
                onChange={(event) => setBreakdownPieceNoun(event.target.value)}
                placeholder={t("workflow.pipelineSettings.piece")}
                className="h-10 w-full max-w-sm"
              />
              <p className="text-xs text-muted-foreground">
                {t("workflow.pipelineSettings.pieceCopyExample", { count: 5, noun: breakdownPieceNounPlural })}
              </p>
            </div>
          </FieldRow>
          <FieldRow label={t("workflow.pipelineSettings.carryOver")}>
            <div className="space-y-2">
              <div className="space-y-1 rounded-md border border-dashed border-border bg-muted/30 px-3 py-2 text-xs">
                <p className="text-muted-foreground">
                  {t("workflow.pipelineSettings.valuesCopied")}
                </p>
                {breakdownTargetPipelineId ? (
                  <div className="flex flex-wrap items-center gap-1 text-muted-foreground">
                    <span>{t("workflow.pipelineSettings.destinationValidation")}</span>
                    <span className="font-medium text-foreground">
                      {breakdownTargetPipeline?.name ?? t("workflow.pipelineSettings.selectedPipeline")}
                    </span>
                ...
                {breakdownIntakeSettingsHref ? (
                  ...
                  >
                    {t("workflow.pipelineSettings.reviewDestinationFields")}
                ...
                {breakdownTargetArchived ? (
                  <p className="flex items-center gap-1 text-amber-700 dark:text-amber-300">
                    ...
                    {t("workflow.pipelineSettings.archivedDestination")}
                  </p>
                ) : null}
              </div>
                            ...
                            >
                              ...
                              {targetField?.required ? (
                                <span className="text-xs text-muted-foreground">
                                  {t("workflow.pipelineSettings.requiredBy", { name: breakdownTargetPipeline?.name ?? t("workflow.pipelineSettings.destination") })}
                                </span>
                              ) : targetField ? (
                                <span className="text-xs text-muted-foreground">
                                  {t("workflow.pipelineSettings.validatedBy", { name: breakdownTargetPipeline?.name ?? t("workflow.pipelineSettings.destination") })}
                                </span>
                              ) : null}
                            </label>
                          );
              ...
              {breakdownCarryOverFieldGroups.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  {t("workflow.pipelineSettings.noCarryOverFields")}
                </p>
              ) : null}
              <p className="text-xs text-muted-foreground">
                {t("workflow.pipelineSettings.uniquePieceFields", { noun: breakdownPieceNoun.trim() || t("workflow.pipelineSettings.piece") })}
              </p>
            </div>
          </FieldRow>
          <FieldRow label={t("workflow.pipelineSettings.moveThen")}>
            <div className="space-y-1">
              <select
              aria-label={t("workflow.pipelineSettings.moveThen")}
              ...
              >
                <option value="">{t("workflow.pipelineSettings.stayOnStep")}</option>
                {otherStages.map((stage) => (
                  <option key={stage.id} value={stage.key}>{stage.name}</option>
                ))}
              </select>
              <p className="text-xs text-muted-foreground">{t("workflow.pipelineSettings.asSoonAsCreated")}</p>
            </div>
          </FieldRow>
          <FieldRow label={t("workflow.pipelineSettings.wait")}>
            <div className="space-y-2">
              <label className="flex items-start gap-2 text-sm">
                ...
                />
                <span className="font-medium text-foreground">
                  {t("workflow.pipelineSettings.waitUntilPiecesFinished", { noun: breakdownPieceNounPlural })}
                </span>
              </label>
              <select
              aria-label={t("workflow.pipelineSettings.moveWhenDone")}
              ...
              {breakdownAdvanceTo ? (
                <p className="text-xs text-muted-foreground">
                  {t("workflow.pipelineSettings.nothingWorthSplittingMove", { name: breakdownCopyNames.advanceToName })}
                </p>
              ) : null}
            </div>
          </FieldRow>
    ...
    <div className="space-y-6">
        ...
        <div className="mb-3 flex items-start justify-between gap-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button type="button" variant="outline" size="icon" className="h-8 w-8" title={t("workflow.pipelines.itemActions")}>
            ...
            <DropdownMenuContent align="end">
              {isArchived ? (
                <DropdownMenuItem onSelect={() => archivePipeline.mutate(false)}>
                  ...
                  {t("workflow.pipelines.resumeNewEntries")}
                </DropdownMenuItem>
              ) : (
                <DropdownMenuItem variant="destructive" onSelect={() => setArchiveDialogOpen(true)}>
                  <Archive className="h-4 w-4" />
                  {t("workflow.pipelineSettings.archive")}
                </DropdownMenuItem>
              )}
            </DropdownMenuContent>
          </DropdownMenu>
        ...
        <div className="grid gap-3 md:grid-cols-(--gtc-13) md:items-end">
          <div className="space-y-3">
            <label className="block space-y-1.5 text-sm font-medium">
              <span className="sr-only">{t("workflow.pipelineSettings.pipelineName")}</span>
              <Input
                aria-label={t("workflow.pipelineSettings.pipelineName")}
                value={pipelineName}
            ...
            <label className="block space-y-1.5 text-sm font-medium">
              <span className="sr-only">{t("workflow.pipelineSettings.pipelineDescription")}</span>
              <Textarea
                aria-label={t("workflow.pipelineSettings.pipelineDescription")}
                value={pipelineDescription}
                onChange={(event) => setPipelineDescription(event.target.value)}
                rows={2}
                placeholder={t("workflow.pipelineSettings.addDescription")}
          ...
          {detailsDirty || savePipelineDetails.isPending ? (
            <Button type="submit" disabled={savePipelineDetails.isPending || !pipelineName.trim()}>
              ...
              {savePipelineDetails.isPending ? t("workflow.common.saving") : t("workflow.pipelineSettings.saveDetails")}
            </Button>
          ) : null}
        </div>
      ...
      <div className="space-y-6">
          {stages.length === 0 ? (
            <EmptyState
              icon={GitBranch}
              message={t("workflow.pipelineSettings.noStages")}
              action={t("workflow.pipelineSettings.addFirstStage")}
                    ...
                    <div key={stage.id} className="flex items-center gap-2">
                      <div className="flex flex-col items-start gap-1">
                        <button
                          ...
                          aria-label={
                            warningCount > 0
                              ? `${stage.name}, ${warningCount} ${warningCount === 1 ? t("workflow.pipelineSettings.warning") : t("workflow.pipelineSettings.warnings")}`
                              : stage.name
                          }
                          className={cn(
                        ...
                        >
                          <span className="flex items-start justify-between gap-2">
                            ...
                            {warningCount > 0 ? (
                              <span className="inline-flex shrink-0 items-center gap-1 text-xs font-semibold text-amber-700 dark:text-amber-300">
                                ...
                                {warningCount} {warningCount === 1 ? t("workflow.pipelineSettings.warning") : t("workflow.pipelineSettings.warnings")}
                              </span>
                            ) : null}
                          </span>
                          <span className="mt-1 block text-xs text-muted-foreground">{t("workflow.pipelineSettings.stepNumber", { number: index + 1 })}</span>
                          {stageNewEntriesDisabled(stage) ? (
                            <span className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-amber-700 dark:text-amber-300">
                              ...
                              {t("workflow.pipelineSettings.newEntriesPaused")}
                            </span>
                          ) : null}
                        </button>
                        <Link
                        ...
                        >
                          {t("workflow.pipelineSettings.viewQueue")}
                      ...
                      {canInsertAfter ? (
                        <button
                          type="button"
                          aria-label={t("workflow.pipelineSettings.insertStageAfter", { name: stage.name })}
                ...
                <div className="min-w-0 flex-1 md:px-8">
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <h2 className="text-lg font-semibold text-foreground">
                      {stageNavigationText(STAGE_SECTION_TITLES[activeStageSection])}
                    </h2>
                    {activeStageSection === "instructions" ? (
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          title={newEntriesDisabled ? t("workflow.pipelines.resumeNewEntries") : t("workflow.pipelines.pauseNewEntries")}
                          aria-label={newEntriesDisabled ? t("workflow.pipelines.resumeNewEntries") : t("workflow.pipelines.pauseNewEntries")}
                  ...
                  {activeStageSection === "instructions" ? (
                    <div className="w-full max-w-3xl">
                      <div className="divide-y divide-border border-b border-border">
                        <FieldRow label={t("workflow.common.name")}>
                          <Input value={stageName} onChange={(event) => setStageName(event.target.value)} required />
                        </FieldRow>
                        <FieldRow label={t("workflow.pipelineSettings.stageType")}>
                          <div className="max-w-xl space-y-2">
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button
                                  type="button"
                                  variant="outline"
                                  aria-label={t("workflow.pipelineSettings.stageType")}
                                ...
                                >
                                  <span className="flex min-w-0 items-center gap-2">
                                    ...
                                    <span className="truncate">{stageKindText(selectedStageKindOption.value, "label")}</span>
                                  </span>
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <span className="min-w-0">
                                          <span className="block font-medium text-foreground">{stageKindText(option.value, "label")}</span>
                                          <span className="mt-0.5 block text-xs leading-5 text-muted-foreground">
                                            {stageKindText(option.value, "description")}
                                          </span>
                                        </span>
                                      </DropdownMenuRadioItem>
                                    );
                            ...
                            <p className="text-sm leading-6 text-muted-foreground">
                              {stageKindText(selectedStageKindOption.value, "description")}
                            </p>
                          </div>
                        </FieldRow>

                        {stageKind === "review" ? (
                          <FieldRow label={t("workflow.pipelineSettings.approver")}>
                            <InlineEntitySelector
                              value={selectedApproval === "any_human" ? "" : selectedApproval}
                              options={approvalOptions}
                              recentOptionIds={recentAssigneeOptionIds}
                              placeholder={t("workflow.pipelineSettings.approver")}
                              noneLabel={t("workflow.pipelineSettings.anyHuman")}
                              searchPlaceholder={t("workflow.pipelineSettings.searchApprovers")}
                              emptyMessage={t("workflow.pipelineSettings.noApprovers")}
                              onChange={(value) => setSelectedApproval(approverValueFromOption(value))}
                              renderTriggerValue={(option) => {
                                if (!option) return <span className="text-muted-foreground">{t("workflow.pipelineSettings.anyHuman")}</span>;
                        ...
                        {stageKind === "review" ? (
                          <FieldRow label={t("workflow.pipelineSettings.reviewOutcomes")}>
                            <div className="space-y-2">
                              {([
                                [t("workflow.pipelineSettings.approvedItemsMoveTo"), approveTarget, setApproveTarget, t("workflow.pipelineSettings.chooseStage")],
                                [t("workflow.pipelineSettings.declinedItemsMoveTo"), rejectTarget, setRejectTarget, t("workflow.pipelineSettings.chooseStage")],
                                [t("workflow.pipelineSettings.itemsNeedingChangesMoveTo"), requestChangesTarget, setRequestChangesTarget, t("workflow.pipelineSettings.stayInReview")],
                              ...
                              <div className="grid grid-cols-1 items-center gap-2 sm:grid-cols-(--gtc-42)">
                                <span className="text-sm font-medium">{t("workflow.pipelineSettings.askRequestNote")}</span>
                                <div className="sm:justify-self-start">
                                  <ToggleSwitch checked={requireRequestChangesReason} onCheckedChange={setRequireRequestChangesReason} />
                                </div>
                              </div>
                              <div className="grid grid-cols-1 items-center gap-2 sm:grid-cols-(--gtc-42)">
                                <span className="text-sm font-medium">{t("workflow.pipelineSettings.askDeclineNote")}</span>
                                <div className="sm:justify-self-start">
                    ...
                    <div className="mt-8 w-full max-w-3xl space-y-6">
                      <div className="overflow-x-auto overscroll-x-contain">
                        <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                          <span>{t("workflow.pipelineSettings.whenEnters")}</span>
                          <InlineEntitySelector
                            value={stageAssigneeOptionId(stageAssigneeAgentId)}
                            options={stageAssigneeOptions}
                            recentOptionIds={recentAssigneeOptionIds}
                            placeholder={t("workflow.pipelineSettings.pickAgent")}
                            noneLabel={t("workflow.pipelineSettings.noAutomation")}
                            searchPlaceholder={t("workflow.pipelineSettings.searchAgents")}
                            emptyMessage={t("workflow.pipelineSettings.noAgents")}
                            onChange={(value) => setStageAssigneeAgentId(stageAssigneeIdFromOption(value))}
                            renderTriggerValue={(option) => {
                              if (!option) return <span className="text-muted-foreground">{t("workflow.pipelineSettings.pickAgent")}</span>;
                          ...
                          />
                          <span>{t("workflow.pipelineSettings.instructions")}</span>
                        </div>
                      </div>
                      ...
                      {selectedAutomationAgent ? (
                        <>
                          <div className="divide-y divide-border border-y border-border">
                            <FieldRow label={t("workflow.pipelineSettings.projectContext")}>
                              <div className="grid gap-2 sm:grid-cols-(--gtc-43)">
                                <InlineEntitySelector
                                  value={stageProjectId}
                                  options={projectOptions}
                                  recentOptionIds={recentProjectIds}
                                  placeholder={t("workflow.pipelineSettings.project")}
                                  noneLabel={t("workflow.pipelineSettings.noProject")}
                                  searchPlaceholder={t("workflow.pipelineSettings.searchProjects")}
                                  emptyMessage={t("workflow.pipelineSettings.noProjects")}
                                  ...
                                  renderTriggerValue={(option) =>
                                    option && selectedAutomationProject ? (
                                      ...
                                      <span className="text-muted-foreground">{t("workflow.pipelineSettings.project")}</span>
                                    )
                                  }
                                  renderOption={(option) => {
                                    if (!option.id) return <span className="truncate">{option.label}</span>;
                                ...
                                {selectedAutomationProject ? (
                                  <select
                                    aria-label={t("workflow.pipelineSettings.projectWorkspace")}
                                    value={stageProjectWorkspaceId}
                                    onChange={(event) => handleAutomationProjectWorkspaceChange(event.target.value)}
                                    className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                                  >
                                      <option value="">{t("workflow.pipelineSettings.projectFallback")}</option>
                                    ...
                                    <div className="flex h-10 items-center rounded-md border border-dashed border-border px-3 text-sm text-muted-foreground">
                                      {t("workflow.pipelineSettings.projectWorkspaceEmpty")}
                                  </div>
                                )}
                              </div>
                              {selectedAutomationProject && !selectedAutomationProjectWorkspace ? (
                                <p className="mt-2 text-xs text-muted-foreground">
                                  {t("workflow.pipelineSettings.projectWorkspaceFallbackDescription")}
                                </p>
                              ) : null}
                            </FieldRow>

                            {selectedAutomationProject && selectedProjectSupportsExecutionWorkspace ? (
                              <FieldRow label={t("workflow.pipelineSettings.executionWorkspace")}>
                                <div className="grid gap-2 sm:grid-cols-(--gtc-43)">
                                  <select
                                    aria-label={t("workflow.pipelineSettings.executionWorkspaceMode")}
                                  ...
                                  >
                                    {STAGE_EXECUTION_WORKSPACE_OPTIONS.map((option) => (
                                      <option key={option.value} value={option.value}>
                                        {workspaceOptionText(option.value)}
                                      </option>
                                    ))}
                                  </select>
                                  {stageExecutionWorkspacePreference === "reuse_existing" ? (
                                    <select
                                      aria-label={t("workflow.pipelineSettings.executionWorkspace")}
                                      value={stageExecutionWorkspaceId}
                                      onChange={(event) => handleAutomationExecutionWorkspaceIdChange(event.target.value)}
                                      className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                                    >
                                      <option value="">{t("workflow.pipelineSettings.chooseWorkspace")}</option>
                                    ...
                                    <div className="flex h-10 items-center rounded-md border border-dashed border-border px-3 text-sm text-muted-foreground">
                                      {stageExecutionWorkspacePreference === "isolated_workspace"
                                        ? t("workflow.pipelineSettings.newWorkspaceCreated")
                                        : t("workflow.pipelineSettings.projectDefaultWorkspace")}
                                    </div>
                                  )}
                                </div>
                                {stageExecutionWorkspacePreference === "reuse_existing" && selectedReusableExecutionWorkspace ? (
                                  <p className="mt-2 text-xs text-muted-foreground">
                                    {t("workflow.pipelineSettings.reusingWorkspace", { name: selectedReusableExecutionWorkspace.name, location: selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? t("workflow.pipelineSettings.existingWorkspace") })}
                                  </p>
                                ) : null}
                                {!canSaveAutomationWorkspace ? (
                                  <p className="mt-2 text-xs text-amber-700 dark:text-amber-300">
                                    {t("workflow.pipelineSettings.chooseWorkspace")}
                                  </p>
                                ) : null}
                              </FieldRow>
                            ) : null}
                          ...
                          <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            ...
                            <span>{t("workflow.pipelineSettings.agentRunsStep", { name: selectedAutomationAgent.name })}</span>
                          </div>
                          <FieldRow label={t("workflow.pipelineSettings.issueTitle")}>
                            <Input
                              ref={issueTitleTemplateInputRef}
                                  aria-label={t("workflow.pipelineSettings.issueTitleTemplate")}
                          ...
                          <AutomationVariableTokenHelper
                            ...
                            label={t("workflow.pipelineSettings.issueTitleTemplateVariables")}
                          />
                          {breakdownEnabled ? (
                            <div className="space-y-1">
              <h3 className="text-sm font-semibold text-foreground">{t("workflow.pipelineSettings.whatDecide")}</h3>
                              <p className="text-sm text-muted-foreground">
                                {t("workflow.pipelineSettings.judgmentDescription")}
                              </p>
                            </div>
                          ) : null}
                          <div data-testid="stage-instructions-editor">
                            <MarkdownEditor
                              ...
                              placeholder={
                                breakdownEnabled
                                  ? t("workflow.pipelineSettings.breakdownPrompt")
                                  : t("workflow.pipelineSettings.instructions")
                              }
                              bordered={false}
                        ...
                        <EmptyState
                          icon={Pause}
                          message={t("workflow.pipelineSettings.emptyAutomation")}
                        />
                      )}
                      <div className="space-y-3">
                  ...
                  {activeStageSection === "advanced" ? (
                    <div className="w-full max-w-3xl space-y-8">
                      <div className="divide-y divide-border border-b border-border">
                        <div className="py-3">
                          <h3 className="text-sm font-semibold text-foreground">{t("workflow.pipelineSettings.transitions")}</h3>
                        </div>
                        <FieldRow label={t("workflow.pipelineSettings.strictMode")}>
                          <div className="space-y-1.5">
                            <div className="flex items-center gap-3">
                              <ToggleSwitch
                                aria-label={t("workflow.pipelineSettings.enforceTransitions")}
                              ...
                              <span className="text-sm font-medium text-foreground">
                                {t("workflow.pipelineSettings.strictlyEnforceTransitions")}
                              </span>
                            </div>
                            <p className="max-w-2xl text-sm text-muted-foreground">
                              {strictTransitionsEnabled
                                ? t("workflow.pipelineSettings.configuredSteps")
                                : t("workflow.pipelineSettings.anyStep")}
                            </p>
                          </div>
                        </FieldRow>
                      ...
                      {isPipelineTerminalStageKind(stageKind) ? null : breakdownEnabled ? (
                        <EmptyState
                          ...
                          message={t("workflow.pipelineSettings.childSettingsHidden")}
                        />
                      ) : (
                        <div className="divide-y divide-border border-b border-border">
                          <div className="py-3">
                            <h3 className="text-sm font-semibold text-foreground">{t("workflow.pipelineSettings.advanceChildren")}</h3>
                          </div>
                          <FieldRow label={t("workflow.pipelineSettings.blockChildren")}>
                            <div className="space-y-1.5">
                              <div className="flex items-center gap-3">
                                <ToggleSwitch
                                ...
                                />
                                <span className="text-sm font-medium text-foreground">
                                  {t("workflow.pipelineSettings.blockChildrenSummary")}
                                </span>
                              </div>
                              <p className="max-w-2xl text-sm text-muted-foreground">
                                {t("workflow.pipelineSettings.blockChildrenDescription")}
                              </p>
                            </div>
                          </FieldRow>
                          <FieldRow label={t("workflow.pipelineSettings.advanceChildren")}>
                            <div className="space-y-3">
                              <div className="flex items-center gap-3">
                                <ToggleSwitch
                                ...
                                />
                                <span className="text-sm font-medium text-foreground">
                                  {t("workflow.pipelineSettings.advanceWhenLastChildDone")}
                                </span>
                              </div>
                              <div className="grid grid-cols-1 items-center gap-2 sm:grid-cols-(--gtc-44)">
                                <span className="text-sm font-medium text-muted-foreground">{t("workflow.pipelines.moveToStage")}</span>
                                <select
                                  aria-label={t("workflow.pipelineSettings.moveWhenDone")}
                    ...
                    <div className="w-full space-y-3">
                      {stageEventsQuery.isLoading ? (
                        ...
                        <StageEventsList
                          events={stageEvents}
                          stages={stages}
                          emptyMessage={t("workflow.pipelineSettings.noStages")}
                        />
                      )}
                    </div>
                  ) : null}
              ...
              {stageDirty || saveStage.isPending ? (
                <div className="sticky bottom-0 z-10 -mx-6 mt-6 flex items-center justify-between gap-3 border-t border-border bg-background/95 px-6 py-3 backdrop-blur motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2">
                  <span className="text-sm text-muted-foreground">
                    {saveStage.isPending ? t("workflow.common.saving") : t("workflow.common.saving")}
                  </span>
                  <Button
                    type="submit"
                  ...
                  >
                    ...
                    {saveStage.isPending ? t("workflow.common.saving") : t("workflow.pipelineSettings.saveStage")}
                  </Button>
                </div>
              ) : null}
            </form>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("workflow.pipelineSettings.deleteStage")}</DialogTitle>
            <DialogDescription>
              {t("workflow.pipelineSettings.deleteStage")} {selectedStage?.name ?? t("workflow.common.stage")}。{t("workflow.pipelineSettings.updateTransitionsFailed")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            {stages.length > 1 ? (
              <label className="block space-y-1.5 text-sm font-medium">
                <span>{t("workflow.pipelines.moveToStage")}</span>
                <select
                  aria-label={t("workflow.pipelines.moveToStage")}
              ...
              <p className="text-sm text-muted-foreground">
                {t("workflow.pipelineSettings.noStages")}
              </p>
            )}
            {deleteStage.error ? (
          ...
          <DialogFooter>
            ...
            >
              <Trash2 className="h-4 w-4" />
              {deleteStage.isPending ? t("workflow.common.deleting") : t("workflow.pipelineSettings.deleteStage")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("workflow.pipelineSettings.archive")}</DialogTitle>
            <DialogDescription>
              {t("workflow.pipelineSettings.archive")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <label className="block space-y-1.5 text-sm font-medium">
              <span>{t("workflow.pipelines.typeToConfirm", { name: pipeline.name })}</span>
              <Input
                aria-label={t("workflow.pipelines.archiveConfirmation")}
          ...
          <DialogFooter>
            ...
            >
              <Archive className="h-4 w-4" />
              {archivePipeline.isPending ? t("workflow.pipelineSettings.archiving") : t("workflow.pipelineSettings.archive")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
