...
import { Camera, Languages, LoaderCircle, Save, Trash2, UserRoundPen } from "lucide-react";
...
import { getLocale, LOCALE_OPTIONS, setLocale, useTranslation, type SupportedLocale } from "@/i18n";
...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("settings.instanceSettings"), href: "/company/settings/instance/general" },
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("profile.board");
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profile.updateProfileError"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("profile.selectCompanyError"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profile.uploadAvatarError"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profile.removeAvatarError"));
    },
  });
  ...
  const currentName = name.trim() || sessionQuery.data.user.name || t("profile.board");
  ...
  const uploadHint = selectedCompany
    ? t("profile.storedForCompany", { company: selectedCompany.name })
          ...
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="max-w-xl text-sm text-muted-foreground">{t("language.description")}</p>
                <p className="text-xs text-muted-foreground">{t("language.saved")}</p>
              </div>
            </div>
            <Select
              value={currentLocale}
            ...
            >
              <SelectTrigger className="w-full sm:w-56" aria-label={t("language.label")}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent align="end" position="popper">
                {LOCALE_OPTIONS.map((option) => (
                  <SelectItem key={option.value} value={option.value}>
                    {option.value === "en" ? t("language.english") : t("language.simplifiedChinese")}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-(--rad-24) border border-border/70 bg-background/92 p-5 shadow-(--shadow-extract-18) backdrop-blur-sm">
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                ...
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("profile.uploadHint", { storageHint: uploadHint })}
                </p>
              </div>
            </div>
          </div>
