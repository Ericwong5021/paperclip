...
export function Projects() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("projectsPage.title") }]);
  }, [setBreadcrumbs, t]);
  ...
  const sortLabel = t(`projectsPage.${sortField}`);

  if (!selectedCompanyId) {
    return <EmptyState icon={Hexagon} message={t("projectsPage.selectCompany")} />;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="sm" className="w-fit text-xs" title={t("issues.sort")}>
              <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
              <span>{t("projectsPage.sort", { field: sortLabel })}</span>
            </Button>
          </PopoverTrigger>
          <PopoverContent align="start" className="w-44 p-0">
            <div className="p-2 space-y-0.5">
              {PROJECT_SORT_OPTIONS.map((option) => (
                ...
                >
                  <span>{t(`projectsPage.${option.field}`)}</span>
                  {sortField === option.field ? (
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      <Check className="h-3 w-3" />
                      {sortDir === "asc" ? t("projectsPage.ascending") : t("projectsPage.descending")}
                    </span>
                  ) : null}
                </button>
              ))}
      ...
      {!isLoading && projects.length === 0 && (
        <EmptyState
          icon={Hexagon}
          message={t("projectsPage.empty")}
          action={t("projectsPage.add")}
      ...
      {projects.length > 0 && (
        <div className="space-y-6">
          {([
            [t("projectsPage.myProjects"), groupedProjects.mine],
            [t("projectsPage.otherProjects"), groupedProjects.other],
              ...
              <section key={label} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {t("projectsPage.projectCount", { count: sectionProjects.length })}
                  </span>
                </div>
                        ...
                        trailing={
                          <div className="flex items-center gap-3">
                            <span
                              ...
                              title={t("projectsPage.taskCount", { count: project.taskCount ?? 0 })}
                            >
                              {t("projectsPage.taskCount", { count: project.taskCount ?? 0 })}
                            </span>
                            {project.budget && (
