...
const concurrencyPolicyDescriptions: Record<string, string> = {
  coalesce_if_active: "routineStatus.policies.coalesceDescription",
  always_enqueue: "routineStatus.policies.enqueueDescription",
  skip_if_active: "routineStatus.policies.skipActiveDescription",
};
const catchUpPolicyDescriptions: Record<string, string> = {
  skip_missed: "routineStatus.policies.skipMissedDescription",
  enqueue_missed_with_cap: "routineStatus.policies.enqueueMissedDescription",
};
...
export function buildRoutineGroups(
  ...
  if (groupByValue === "folder") {
    ...
    return Object.keys(groups)
      .sort((left, right) => {
        ...
        const labelCompare = (leftFolder?.name ?? t("routineStatus.common.unknown")).localeCompare(
          rightFolder?.name ?? t("routineStatus.common.unknown"),
          undefined,
          { sensitivity: "base" },
        );
      ...
      .map((key) => ({
        key,
        label: key === "__unfiled" ? t("routineStatus.routinePage.unfiled") : (folderById.get(key)?.name ?? t("routineStatus.common.unknown")),
        items: groups[key]!,
      }));
  }
  ...
  if (groupByValue === "project") {
    ...
    return Object.keys(groups)
      .sort((left, right) => {
        const leftLabel = left === "__no_project" ? t("routineStatus.common.none") : (projectById.get(left)?.name ?? t("routineStatus.common.unknown"));
        const rightLabel = right === "__no_project" ? t("routineStatus.common.none") : (projectById.get(right)?.name ?? t("routineStatus.common.unknown"));
        return leftLabel.localeCompare(rightLabel);
      })
      .map((key) => ({
        key,
        label: key === "__no_project" ? t("routineStatus.routinePage.noProject") : (projectById.get(key)?.name ?? t("routineStatus.common.unknown")),
        items: groups[key]!,
      }));
  }
  ...
  return Object.keys(groups)
    .sort((left, right) => {
      const leftLabel = left === "__unassigned" ? t("routineStatus.common.none") : (agentById.get(left)?.name ?? t("routineStatus.common.unknown"));
      const rightLabel = right === "__unassigned" ? t("routineStatus.common.none") : (agentById.get(right)?.name ?? t("routineStatus.common.unknown"));
      return leftLabel.localeCompare(rightLabel);
    })
    .map((key) => ({
      key,
      label: key === "__unassigned" ? t("routineStatus.common.none") : (agentById.get(key)?.name ?? t("routineStatus.common.unknown")),
      items: groups[key]!,
    }));
}
...
export function Routines() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("routinesPage.title") }]);
  }, [setBreadcrumbs, t]);
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routineStatus.routinePage.routineCreated"),
        body: routine.assigneeAgentId
          ? t("routineStatus.routinePage.addFirstTrigger")
          : t("routineStatus.routinePage.draftSaved"),
        tone: "success",
      });
    ...
    onSuccess: async (folder) => {
      ...
      if (moveAfterCreateIds.length > 0) {
        ...
        try {
          ...
          pushToast({
            title: t("routineStatus.routinePage.folderCreatedMoveFailed"),
            body: moveError instanceof Error ? moveError.message : t("routineStatus.routinePage.couldNotMoveSelected"),
            tone: "error",
          });
          return;
        }
      ...
      pushToast({ title: t("routineStatus.routinePage.folderCreated"), body: folder.name, tone: "success" });
    },
    onError: (mutationError) => {
      pushToast({
        title: t("routineStatus.routinePage.failedToSaveFolder"),
        body: mutationError instanceof Error ? mutationError.message : t("routineStatus.routinePage.couldNotSaveFolder"),
        tone: "error",
      });
    },
  });
  const updateFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routineStatus.routinePage.folderSaveFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routineStatus.routinePage.couldNotUpdateFolder"),
        tone: "error",
      });
    },
  });
  const deleteFolder = useMutation({
    ...
    onSuccess: async (_, folderId) => {
      ...
      pushToast({ title: t("routineStatus.routinePage.folderDeleted"), body: t("routineStatus.routinePage.itemsMovedToUnfiled"), tone: "success" });
    },
    onError: (mutationError) => {
      pushToast({
        title: t("routineStatus.routinePage.folderDeleteFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routineStatus.routinePage.couldNotDeleteFolder"),
        tone: "error",
      });
    },
  });
  const moveRoutineToFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routineStatus.routinePage.moveFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routineStatus.routinePage.couldNotMoveRoutine"),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routineStatus.routinePage.routineUpdated"),
        body: mutationError instanceof Error ? mutationError.message : t("routineStatus.routinePage.couldNotUpdateRoutine"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routineStatus.routinePage.routineRunFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routineStatus.routinePage.couldNotStartRun"),
        tone: "error",
      });
    },
  });
  ...
  async function moveSelectedRoutines(folderId: string | null) {
    ...
    try {
      ...
      pushToast({
        title: t("routineStatus.routinePage.moved"),
        body: t(ids.length === 1 ? "routineStatus.routinePage.filed" : "routineStatus.routinePage.filed_other", { count: ids.length }),
        tone: "success",
      });
    } catch (moveError) {
      pushToast({
        title: t("routineStatus.routinePage.failedToMove"),
        body: moveError instanceof Error ? moveError.message : t("routineStatus.routinePage.couldNotMoveSelected"),
        tone: "error",
      });
    }
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    if (!enabled && !routine.assigneeAgentId) {
      pushToast({
        title: t("routineStatus.routinePage.defaultAgentRequired"),
        body: t("routineStatus.routinePage.setDefaultBeforeEnable"),
        tone: "warn",
      });
      return;
    }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t("routinesPage.selectCompany")} />;
  }
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <PageTabBar
          ...
          items={[
            ...
            { value: "runs", label: t("routinesPage.recentRuns") },
          ]}
        />
            ...
            <div className="flex items-center gap-1">
              <Popover>
                ...
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["updated", t("routineStatus.routinePage.sortUpdated")],
                      ["created", t("routineStatus.routinePage.sortCreated")],
                      ["lastRun", t("routineStatus.routinePage.sortLastRun")],
                      ["title", t("routineStatus.routinePage.sortTitle")],
                      ...
                      >
                        ...
                        {routineViewState.sortField === field ? (
                          <span className="text-xs text-muted-foreground">
                            {routineViewState.sortDir === "asc" ? t("routineStatus.routinePage.ascending") : t("routineStatus.routinePage.descending")}
                          </span>
                        ) : null}
                      </button>
                    ))}
              ...
              <Popover>
                ...
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["folder", t("routineStatus.routinePage.folder")],
                      ["project", t("routineStatus.common.project")],
                      ["assignee", t("routineStatus.common.agent")],
                      ["none", t("routineStatus.common.none")],
          ...
          {routineViewState.groupBy === "folder" ? (
            <div className="md:hidden">
              <FolderChip
                ...
                allLabel={t("routineStatus.routinePage.allRoutines")}
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">{t("routineStatus.routinePage.newRoutine")}</p>
              <p className="text-sm text-muted-foreground">
                {t("routineStatus.routinePage.composerDescription")}
              </p>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="px-5 pt-5 pb-3">
              <textarea
                ...
                placeholder={t("routineStatus.routinePage.titlePlaceholder")}
                rows={1}
                value={draft.title}
                onChange={(event) => {
            ...
            <div className="px-5 pb-3">
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  ...
                  />
                  <span>{t("routineStatus.routinePage.filedIn")}</span>
                  <Select
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="__unfiled">{t("routineStatus.routinePage.unfiled")}</SelectItem>
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">{t("routineStatus.routinePage.advancedDelivery")}</p>
                    <p className="text-sm text-muted-foreground">{t("routineStatus.routinePage.deliveryHint")}</p>
                  </div>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">{t("routineStatus.routinePage.concurrency")}</p>
                      <Select
                    ...
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">{t("routineStatus.routinePage.catchUp")}</p>
                      <Select
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              {t("routineStatus.routinePage.afterCreateHint")}
            </div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? t("routineStatus.routinePage.creating") : t("routineStatus.routinePage.create")}
              </Button>
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("routineStatus.routinePage.failedToCreate")}
                </p>
              ) : null}
            </div>
          </div>
      ...
      {error ? (
        <Card>
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : t("routineStatus.routinePage.failedToLoad")}
          </CardContent>
        </Card>
      ) : null}
        ...
        <div className={cn(showFolderRail && "flex gap-4")}>
          {showFolderRail ? (
            <FolderRail
              ...
              selection={folderSelection}
              allLabel={t("routineStatus.routinePage.allRoutines")}
              itemLabelPlural={t("routineStatus.common.routines")}
          ...
          {routineViewState.groupBy === "folder" && hasRoutineFolders ? (
            <div className="mb-3 flex flex-wrap items-center gap-2">
              {folderSelection === "all" ? <FolderIconHeader label={t("routineStatus.routinePage.allRoutines")} count={sortedRoutines.length} /> : (
                <div className="flex min-w-0 items-center gap-2 text-sm">
                  <FolderSwatch color={activeFolder?.color} />
                  <span className="truncate font-medium">{folderSelection === "unfiled" ? t("routineStatus.routinePage.unfiled") : activeFolder?.name ?? t("routineStatus.routinePage.folder")}</span>
                  <span className="text-muted-foreground">{t("routineStatus.routinePage.routineCount", { count: sortedRoutines.length })}</span>
                </div>
              )}
            </div>
          ) : null}
          ...
          {visibleRoutines.length === 0 ? (
            ...
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message={folderSelection === "all" ? t("routineStatus.routinePage.noRoutinesMatch") : t("routineStatus.routinePage.folderEmpty")}
              />
              {folderSelection !== "all" ? (
                <div className="mt-3 flex justify-center">
                  <Button size="sm" onClick={openCreateRoutine}>
                    <Plus className="mr-2 h-3.5 w-3.5" />
                    {t("routineStatus.routinePage.newRoutineInFolder")}
                  </Button>
                </div>
              ) : null}
            </div>
                          ...
                          extraMenuItems={
                            <MoveToMenu
                              ...
                              onMove={(folderId) => {
                                ...
                                pushToast({
                                  title: t("routineStatus.routinePage.routineMoved"),
                                  body: folderId
                                    ? t("routineStatus.routinePage.movedTo", { title: routine.title, folder: routineFolders?.folders.find((folder) => folder.id === folderId)?.name ?? t("routineStatus.routinePage.folder") })
                                    : t("routineStatus.routinePage.movedTo", { title: routine.title, folder: t("routineStatus.routinePage.unfiled") }),
                                  tone: "success",
                                  action: {
                                    label: t("routineStatus.routinePage.undo"),
...
function FolderIconHeader({ label, count }: { label: string; count: number }) {
  return (
    <div className="flex min-w-0 items-center gap-2 text-sm">
      ...
      <span className="text-muted-foreground">{t("routineStatus.routinePage.routineCount", { count })}</span>
    </div>
  );
}
