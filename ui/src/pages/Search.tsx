...
const SUBGROUP_LABEL_KEYS: Record<SubGroupKey, string> = {
  issues: "search.resultGroups.issues",
  comments: "search.resultGroups.comments",
  documents: "search.resultGroups.documents",
  artifacts: "search.resultGroups.artifacts",
  agents: "search.resultGroups.agents",
  projects: "search.resultGroups.projects",
};
...
function subgroupLabel(group: SubGroupKey): string {
  return t(SUBGROUP_LABEL_KEYS[group]);
}
...
function describeScope(scope: CompanySearchScope) {
  if (scope === "all") return t("search.scope.allScopes");
...
function shapeError(error: unknown): { message: string; status?: number } {
  if (!error) return { message: t("search.errors.unknown") };
...
export function Search() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: translate("search.title") }]);
  ...
  }, [counts, data, filtersActive, translate]);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col" data-page="search">
      <div className="border-b border-border px-4 py-3 sm:px-6">
          <h1 className="sr-only">{translate("search.title")}</h1>
        <div className="relative">
          ...
          <Input
            ...
            aria-label={translate("search.input.queryAria")}
          ...
          {draftQuery.length > 0 ? (
            <button
              type="button"
              onClick={handleClear}
              aria-label={translate("search.input.clearAria")}
        ...
        <div className="mt-2 flex min-h-6 flex-wrap items-center gap-1.5 text-(length:--text-micro) text-muted-foreground">
          ...
          {operatorSuggestions.length > 0 ? (
            <div className="flex flex-wrap items-center gap-1.5" data-testid="search-operator-suggestions">
              {operatorSuggestions.map((suggestion) => (
                <button
                  ...
                  type="button"
                  aria-label={translate("search.input.insertOperator", { token: suggestion.token })}
            ...
            <span className="truncate">
              {translate("search.input.tryPrefix")} <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">status:todo</code>,{" "}
              <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">assignee:me</code>,{" "}
              {translate("search.input.or")} <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">updated:&gt;7d</code>.
            </span>
          )}
        </div>
      </div>
  ...
  if (showInitialState) {
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        <div>
          <h2 className="text-lg font-semibold">{translate("search.initial.heading")}</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            {translate("search.initial.description")}
          </p>
        </div>
        {recentSearches.length > 0 ? (
          <div>
            <div className="mb-2 text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
              {translate("search.initial.recent")}
            </div>
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <span className="font-medium text-foreground">{translate("search.initial.identifierLookup")}:</span>{" "}
            {translate("search.initial.identifierHelp", {
              identifier: "PAP-123",
            }).split("PAP-123")[0]}
            <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">PAP-123</code>
            {translate("search.initial.identifierHelp", { identifier: "PAP-123" }).split("PAP-123")[1]}
          </li>
          <li>
            <span className="font-medium text-foreground">{translate("search.initial.quotedPhrases")}:</span>{" "}
            {translate("search.initial.quotedHelp")}
          </li>
          <li>
            <span className="font-medium text-foreground">{translate("search.initial.commandShortcut")}:</span>{" "}
            {translate("search.initial.commandHelp")}
          </li>
        </ul>
      </div>
    );
  ...
  if (hasError) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">{translate("search.errors.heading")}</div>
        <p className="text-sm text-muted-foreground">
          {status
            ? translate("search.errors.serverReturned", { status })
            : translate("search.errors.requestFailed")} {translate("search.errors.preserved")}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          <Button onClick={refetch} variant="default" size="sm">
            {translate("search.errors.retry")}
          </Button>
          <Button onClick={navigateIssuesFallback} variant="outline" size="sm">
            {translate("search.errors.openTasks")}
          </Button>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex flex-col gap-2 px-2 py-3 sm:px-4">
        <div className="px-3 text-xs text-muted-foreground" data-testid="search-loading">
          {translate("search.loading.searching", { query: trimmedQuery })}
        </div>
  ...
  if (isEmpty) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">{translate("search.empty.heading", { query: trimmedQuery })}</div>
        <p className="text-sm text-muted-foreground">
          {translate("search.empty.description", { scope: describeScope(scope).toLowerCase() })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
            <Button onClick={showAllScope} size="sm" variant="outline">
              {translate("search.empty.allScopes")}
            </Button>
          ) : null}
          <Button onClick={openNewIssue} size="sm" variant="default">
            <Plus className="mr-1.5 h-4 w-4" />
            {translate("search.empty.createTask")}
          </Button>
        ...
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          <li>{translate("search.empty.fewerTokens")}</li>
          <li>
            {translate("search.empty.identifierShortcut", { identifier: "PAP-123" }).split("PAP-123")[0]}
            <code className="rounded bg-muted px-1 py-0.5">PAP-123</code>
            {translate("search.empty.identifierShortcut", { identifier: "PAP-123" }).split("PAP-123")[1]}
          </li>
          <li>{translate("search.empty.quotedPhrases")}</li>
        </ul>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between py-2 text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
        <span>
          {allMatchTotal > totalResults
            ? translate("search.results.range", { shown: totalResults, total: allMatchTotal })
            : totalResults === 1
              ? translate("search.results.one")
              : translate("search.results.many", { count: totalResults })}
          {` · ${translate("search.results.sortedBy", { sort: sortLabel })}`}
          {activeFilterCount > 0
            ? ` · ${translate(activeFilterCount === 1 ? "search.results.activeFilter" : "search.results.activeFilters", { count: activeFilterCount })}`
            : ""}
        </span>
        {isFetching ? <span aria-live="polite" className="normal-case tracking-normal">{translate("search.results.updating")}</span> : null}
      </div>
