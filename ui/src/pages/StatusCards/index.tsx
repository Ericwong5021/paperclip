...
export function StatusCards() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("routineStatus.statusCards.title") }]);
  }, [setBreadcrumbs]);
    ...
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <h1 className="text-xl font-bold">{t("routineStatus.statusCards.title")}</h1>
          <Badge variant="secondary" className="gap-1">
            <FlaskConical className="h-3 w-3" />
            {t("routineStatus.statusCards.experimental")}
          </Badge>
        </div>
        <div className="flex items-center gap-4">
          {showCostMeter ? (
            <span className="text-xs text-muted-foreground">
              {t("routineStatus.statusCards.today")}: {formatTokens(todayTotals.tokens)} · ~{formatCents(todayTotals.cents)}
            </span>
          ) : null}
          <Button onClick={() => setCreateOpen(true)} disabled={!selectedCompanyId}>
            <Plus className="h-4 w-4" />
            {t("routineStatus.statusCards.newCard")}
          </Button>
        </div>
      </div>

      {actionError ? <InlineBanner tone="warning" title={t("routineStatus.statusCards.headsUp")}>{actionError}</InlineBanner> : null}

      {activeQuery.isLoading ? (
        <div className="flex items-center gap-2 py-12 text-sm text-muted-foreground">
          <Loader2 className="h-4 w-4 animate-spin" /> {t("routineStatus.common.loading")} {t("routineStatus.statusCards.statusCard").toLowerCase()}…
        </div>
      ) : activeQuery.isError ? (
        <InlineBanner tone="danger" title={t("routineStatus.statusCards.loadFailed")}>
          {activeQuery.error instanceof Error ? activeQuery.error.message : t("routineStatus.common.retry")}
        </InlineBanner>
      ) : activeCards.length === 0 ? (
        <EmptyState
          icon={FlaskConical}
          title={t("routineStatus.statusCards.noCards")}
          message={t("routineStatus.statusCards.noCardsMessage")}
          action={selectedCompanyId ? t("routineStatus.statusCards.newCard") : undefined}
      ...
      {archivedCards.length > 0 ? (
        <div className="space-y-3 pt-2">
          ...
          >
            {showArchived ? t("routineStatus.statusCards.hideArchived") : t("routineStatus.statusCards.showArchived", { count: archivedCards.length })}
          </button>
          {showArchived
