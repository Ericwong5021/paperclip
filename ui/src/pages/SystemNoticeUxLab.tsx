...
function localizeNotice(fixture: (typeof systemNoticeFixtures)[number]) {
  return {
    ...fixture,
    caption: labT(fixture.caption),
    label: typeof fixture.label === "string" ? labT(fixture.label) : fixture.label,
    source: fixture.source
      ? { ...fixture.source, label: typeof fixture.source.label === "string" ? labT(fixture.source.label) : fixture.source.label }
      : fixture.source,
    body: typeof fixture.body === "string" ? labT(fixture.body) : fixture.body,
    metadata: fixture.metadata?.map((section) => ({
      ...section,
      title: typeof section.title === "string" ? labT(section.title) : section.title,
      rows: section.rows.map((row) => ({
        ...row,
        label: typeof row.label === "string" ? labT(row.label) : row.label,
        ...(typeof row === "object" && "title" in row && typeof row.title === "string"
          ? { title: labT(row.title) }
          : {}),
        ...(typeof row === "object" && "value" in row && typeof row.value === "string"
          ? { value: labT(row.value) }
          : {}),
      })),
    })),
  };
...
export function SystemNoticeUxLab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <LabSection
        id="tones"
        eyebrow={labT("Tone matrix")}
        title={labT("Three tones, two states")}
        description={labT("Each tone pairs a unique icon and tone label so the notice is recognizable without color. Collapsed is the default; the Details affordance reveals operational metadata only when reviewers ask for it.")}
        accentClassName="bg-[linear-gradient(180deg,rgba(245,158,11,0.05),transparent_28%),var(--background)]"
      >
        <div className="space-y-5">
          <FixtureFrame caption={labT(warningCollapsed.caption)}>
            <SystemNotice {...localizeNotice(warningCollapsed)} />
          </FixtureFrame>
          <FixtureFrame caption={labT(warningExpanded.caption)}>
            <SystemNotice {...localizeNotice(warningExpanded)} />
          </FixtureFrame>
          <FixtureFrame caption={labT(dangerCollapsed.caption)}>
            <SystemNotice {...localizeNotice(dangerCollapsed)} />
          </FixtureFrame>
          <FixtureFrame caption={labT(dangerExpanded.caption)}>
            <SystemNotice {...localizeNotice(dangerExpanded)} />
          </FixtureFrame>
          <FixtureFrame caption={labT(neutralCollapsed.caption)}>
            <SystemNotice {...localizeNotice(neutralCollapsed)} />
          </FixtureFrame>
          <FixtureFrame caption={labT(neutralExpanded.caption)}>
            <SystemNotice {...localizeNotice(neutralExpanded)} />
          </FixtureFrame>
          <FixtureFrame caption={labT(warningNoDetails.caption)}>
            <SystemNotice {...localizeNotice(warningNoDetails)} />
          </FixtureFrame>
        </div>
      </LabSection>
      ...
      <LabSection
        id="hierarchy"
        eyebrow={labT("Hierarchy in thread")}
        title={labT("Distinct from user and agent comments")}
        description={labT("Side-by-side with adjacent comment types so reviewers can confirm the system row reads as a system row — full width, no avatar gutter, no chat bubble — while user and agent comments keep their existing rounded bubbles.")}
      ...
      >
        <div className="space-y-4 rounded-2xl border border-border/70 bg-background/70 p-4">
          <MockUserBubble
            ...
            body={labT("Why does this issue keep waking back up without a clear next step?")}
            alignEnd
          />
          <MockAgentBubble
            agentName="CodexCoder"
            body={labT("The previous run completed without picking a disposition. I'll wait for the new system notice to surface so the recovery owner is unambiguous.")}
          />
          <SystemNotice
            tone="danger"
            label={labT("System alert")}
            source={{ label: "Paperclip", href: "/PAP/agents" }}
            timestamp="2026-05-04T16:48:00.000Z"
            body={labT("Paperclip could not resolve this issue's missing disposition automatically. The issue is blocked on a recovery owner.")}
            metadata={[
              {
          ...
          <MockUserBubble
            authorName="Riley Board"
            body={labT("Thanks — assigning the recovery owner now.")}
            alignEnd
          />
        </div>
      </LabSection>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <LabSection
          eyebrow={labT("Before")}
          title={labT("Today's nested treatment")}
          description={labT("The same content rendered through the existing user-bubble + warning-callout path. Two containers, same gray background as user comments, and the warning icon is forced inside a chat row.")}
        ...
        >
          <div className="space-y-3 rounded-2xl border border-border/70 bg-background/70 p-4">
            <div className="flex items-start gap-2.5">
              ...
              <div className="flex min-w-0 max-w-(--pct-85) flex-col">
                <div className="mb-1 px-1 text-sm font-medium text-foreground">{labT("You")}</div>
                <div className="min-w-0 max-w-full rounded-2xl bg-muted px-4 py-2.5 text-sm leading-6 text-foreground">
                  <div className="rounded-md border border-red-500/35 bg-red-500/10 px-3 py-2.5 text-sm text-red-950 dark:text-red-100">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0">
                        <p className="m-0 font-semibold">{labT("Successful run handoff missing")}</p>
        ...
        <LabSection
          eyebrow={labT("After")}
          title={labT("System notice replacement")}
          description={labT("One container, system-authored label, hidden details. The chat surface keeps user and agent bubbles unchanged.")}
          accentClassName="bg-[linear-gradient(180deg,rgba(16,185,129,0.05),transparent_28%),var(--background)]"
        >
          <div className="space-y-3 rounded-2xl border border-border/70 bg-background/70 p-4">
            <SystemNotice {...dangerCollapsed} />
            <p className="px-1 text-xs text-muted-foreground">
              {labT("Same content. The visible body is one short system sentence; reviewers expand")} {" "}
              <span className="font-medium text-foreground">{labT("Details")}</span> {labT("only when they need run")}
      ...
      <Card className="gap-4 border-border/70 bg-background/85 py-0">
        <CardHeader className="px-5 pt-5 pb-0">
          ...
          <CardTitle className="text-lg">{labT("Handoff to engineering")}</CardTitle>
          <CardDescription>
        ...
        <CardContent className="space-y-3 px-5 pb-5 pt-0 text-sm text-muted-foreground">
          <div className="rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
            <div className="mb-1 font-medium text-foreground">{labT("Component")}</div>
          ...
          <div className="rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
            <div className="mb-1 font-medium text-foreground">{labT("Accessibility")}</div>
          ...
          <div className="rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
            <div className="mb-1 font-medium text-foreground">{labT("Legacy fallback")}</div>
