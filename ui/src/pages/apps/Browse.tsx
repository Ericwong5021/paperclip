...
export function Browse() {
  ...
  if (!selectedCompanyId) {
    return <div className="p-6 text-sm text-muted-foreground">{t("appsTools.selectCompanyBrowse")}</div>;
  }
  ...
  return (
    <div className="max-w-5xl space-y-8 pb-12">
      <header>
        <h1 className="text-2xl font-bold tracking-tight">{t("appsTools.browse")}</h1>
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          aria-label={t("appsTools.searchApps")}
      ...
      {loading ? (
        ...
        <>
          ...
          <section className="space-y-3">
            <div className="text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
              {trimmed ? t("appsTools.results", { count: filtered.length }) : t("appsTools.allAppsCount")}
            </div>
            {filtered.length === 0 ? (
              <p className="flex items-center gap-1.5 rounded-xl border border-dashed border-border bg-card px-4 py-6 text-sm text-muted-foreground">
                ...
                {t("appsTools.noPlannedApps", { query: query.trim() })}
              </p>
            ) : (
    ...
    return (
      ...
      >
        ...
        <span className={disabled ? "text-xs text-muted-foreground" : "text-xs font-semibold text-primary"}>
          {disabled ? t("appsTools.comingSoon") : t("appsTools.connectArrow")}
        </span>
      </button>
    );
  }
